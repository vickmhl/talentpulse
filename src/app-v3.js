(function () {
  ...
  function buildIssues(rows, summary) {
    ...
    return [makeInsight("组织需要优先核验关键岗位覆盖", "当前没有极端异常，但关键岗位覆盖和高潜转化仍值得优先确认。", 70, "succession", "如果不持续滚动复核，继任暴露会在组织调整时被放大。", "先核验高暴露岗位后备名单。", "建立 ready soon 候选池。", "形成周期性的继任盘点机制。")];
  }
  ...
  function renderOverview() {
    ...
    const presenter = state.uploadMode === "demo" ? `<details class="card presenter-note"><summary>Presenter note</summary><p>先用 headline 定调，再讲 3 个 strongest issues，最后用 immediate move 收口并跳到证据页。</p></details>` : "";
    return `<section class="executive-hero card"><div class="executive-main"><div class="tag">Executive Summary</div><h2>${safe(summary.headline)}</h2><p>${safe(summary.state)}</p><div class="summary-kpis"><div><span>Risk level</span><strong>${safe(summary.riskLevel)}</strong></div><div><span>Immediate move</span><strong>${safe(summary.priority)}</strong></div></div></div><div class="executive-side">${summary.issues.slice(0, 3).map((item) => `<div class="summary-point"><strong>${safe(item.title)}</strong><span>${safe(item.detail)}</span></div>`).join("")}</div></section>${presenter}<section class="card"><div class="section-head"><h3>Current State</h3><span>4 strongest evidence only</span></div><div class="grid-kpi">${stateCards.map((item) => `<article class="card kpi-card"><span>${safe(item.label)}</span><strong>${safe(item.value)}</strong><small>${safe(item.note)}</small></article>`).join("")}</div><div class="section-grid-2 push-top"><article class="card"><div class="section-head"><h3>Department difference</h3><span>只保留最有故事的团队</span></div><div class="story-grid">${departmentStoryCards(summary).slice(0, 4).map((item) => `<div><strong>${safe(item.title)}</strong><p>${safe(item.detail)}</p></div>`).join("")}</div></article><article class="card"><div class="section-head"><h3>9-box evidence</h3><span>九宫格是证据，不是主角</span></div>${renderBars(ninebox.filter((cell) => cell.employees.length).sort((a, b) => b.employees.length - a.employees.length).slice(0, 4).map((cell) => ({ label: cell.title, value: cell.employees.length, color: cell.key === "3-3" ? "var(--primary)" : cell.key === "3-2" ? "#4a7cff" : "var(--warning)" })))}<div class="priority-card push-top"><strong>How to read</strong><p>右上格代表核心高潜，右中格代表高绩效但接班尚未成熟的人群。</p></div></article></div></section><section class="card"><div class="section-head"><h3>Key Issues</h3><span>3 strongest cards</span></div><div class="section-grid-3">${issueCards.slice(0, 3).map((item) => `<article class="card insight-card"><div class="insight-line"><strong>${safe(item.title)}</strong><p>${safe(item.phenomenon)}</p></div><div class="insight-line"><strong>Why this matters</strong><p>${safe(item.likelyConsequence)}</p></div><div class="insight-line"><strong>Management read</strong><p>${safe(item.managementRead)}</p></div></article>`).join("")}</div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Why It’s Happening</h3><span>真正的 cause cards</span></div>${issueCards.slice(0, 3).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.rootCause)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>Priority Focus</h3><span>现在最先盯两件事</span></div>${actions.immediate.slice(0, 2).map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(`${item.target} · ${item.purpose}`)}</p></div>`).join("")}</article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Risk Outlook</h3><span>6-12 month outlook</span></div>${signals.slice(0, 4).map((text) => `<div class="priority-card"><strong>${safe(text)}</strong></div>`).join("")}</article><article class="card"><div class="section-head"><h3>Recommended Actions</h3><span>Immediate / Mid-term / Bench-building</span></div><div class="time-list"><div><strong>Immediate</strong><p>${safe(actions.immediate.map((item) => item.action).join(" "))}</p></div><div><strong>Mid-term</strong><p>${safe(actions.mid.map((item) => item.action).join(" "))}</p></div><div><strong>Bench-building</strong><p>${safe(actions.bench.map((item) => item.action).join(" "))}</p></div></div></article></section>${renderGuideCards([{ route: "review", label: "Talent Review", reason: "去看人才结构与伪强项证据" }, { route: "succession", label: "Succession", reason: "去看覆盖、空窗与单点依赖证据" }, { route: "health", label: "Data Health", reason: "去看 AI 如何理解并修复数据" }, { route: "report", label: "Report", reason: "进入完整汇报稿" }])}`;
  }
  function renderTalentReview() {
    ...
    return `<section class="executive-summary card"><div class="executive-main"><div class="tag">Talent Snapshot</div><h2>${safe(summary.kpis.hipoA > summary.kpis.readyNow ? "Bench maturity is weaker than performance signals suggest" : "Talent density is usable, but not evenly converted into bench strength")}</h2><p>${safe(`当前样本 ${rows.length} 人。高潜 A ${summary.kpis.hipoA}，Ready Now ${summary.kpis.readyNow}，高绩效但未 ready-now ${countBy(rows, (row) => scorePerformance(row.performance_current) >= 4 && row.succession.band !== "Ready Now")} 人。`)}</p></div><div class="executive-side"><div class="summary-point"><strong>What this page proves</strong><span>人才密度、结构断层、伪强项。</span></div></div></section><section class="section-grid-3"><article class="card"><div class="section-head"><h3>Structural finding 1</h3><span>talent concentration</span></div><div class="priority-card"><strong>${safe(departmentStoryCards(summary)[0].title)}</strong><p>${safe(departmentStoryCards(summary)[0].detail)}</p></div></article><article class="card"><div class="section-head"><h3>Structural finding 2</h3><span>misleading strength</span></div><div class="priority-card"><strong>${safe(misleading[0].title)}</strong><p>${safe(misleading[0].detail)}</p></div></article><article class="card"><div class="section-head"><h3>Structural finding 3</h3><span>bench gap</span></div><div class="priority-card"><strong>${safe(topIssueCards(summary)[0].title)}</strong><p>${safe(topIssueCards(summary)[0].phenomenon)}</p></div></article></section><section class="card"><div class="section-head"><h3>9-box Evidence</h3><span>组织证据先于个体样本</span></div><div class="filter-row"><select class="filter-select" data-filter="department"><option value="All">全部部门</option>${unique(state.employees.map((row) => row.department)).map((department) => `<option value="${safe(department)}" ${state.selectedDepartment === department ? "selected" : ""}>${safe(department)}</option>`).join("")}</select><select class="filter-select" data-filter="jobLevel"><option value="All">全部职级</option>${unique(state.employees.map((row) => row.job_level)).map((level) => `<option value="${safe(level)}" ${state.selectedJobLevel === level ? "selected" : ""}>${safe(level)}</option>`).join("")}</select><span class="status info">当前样本 ${rows.length} 人</span></div><div class="ninebox-grid push-top">${cells.map((cell) => `<button class="ninebox-cell ${selectedCell && selectedCell.key === cell.key ? "active" : ""}" data-ninebox="${safe(cell.key)}"><span class="cell-title">${safe(cell.title)}</span><strong>${safe(cell.employees.length)}</strong><small>${safe(cell.key === "3-3" ? "高潜集中区" : cell.key === "3-2" ? "高绩效但梯队未成熟" : cell.key === "2-1" ? "稳定但成长有限" : "结构证据")}</small></button>`).join("")}</div><div class="priority-card push-top"><strong>${safe(selectedCell && selectedCell.key === "3-3" ? "高潜集中并不等于梯队成熟" : selectedCell && selectedCell.key === "3-2" ? "高绩效集中区暴露出接班不足" : "当前格子更适合说明结构差异")}</strong><p>${safe(selectedCell && selectedCell.employees.length ? `当前格子 ${selectedCell.employees.length} 人。` : "当前格子暂无代表员工。")}</p></div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Department Judgment Cards</h3><span>只讲最有故事的 4 个部门</span></div><div class="story-grid">${focusDepartments.map((department) => `<div><strong>${safe(department)}</strong><p>${safe(`${departments[department].count} 人，高潜 ${departments[department].hipoA}，Ready Now ${departments[department].readyNow}，高绩效 ${departments[department].highPerf}。`)}</p></div>`).join("")}</div></article><article class="card"><div class="section-head"><h3>Misleading Strengths</h3><span>伪强项比弱项更危险</span></div>${misleading.map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.detail)}</p></div>`).join("")}</article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Bench Gaps</h3><span>哪里存在梯队断层</span></div>${topIssueCards(summary).filter((item) => /bench|potential|readiness|strength|梯队|高潜|成熟/.test((item.title + item.rootCause + item.managementRead).toLowerCase())).slice(0, 3).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.phenomenon)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>AI Talent Insights</h3><span>可直接拿来讲的判断</span></div>${departmentStoryCards(summary).slice(0, 4).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.detail)}</p></div>`).join("")}</article></section>${renderProfileDrawer(selectedEmployee, "查看人才样本")}`;
  }
  ...
  function renderSuccession() {
    ...
    return `<section class="executive-summary card"><div class="executive-main"><div class="tag">Succession Health Summary</div><h2>${safe(summary.kpis.coverage < 70 ? "Coverage is visible, but bench depth is still shallow" : "Coverage is usable, but too many roles still rely on one candidate")}</h2><p>${safe(`关键岗位 ${summary.kpis.criticalRoles} 个，已覆盖 ${summary.kpis.coveredCriticalRoles} 个，Ready Now ${summary.kpis.readyNow} 人。`)}</p></div><div class="executive-side"><div class="summary-point"><strong>What this page proves</strong><span>覆盖、空窗、单点依赖、支持部门风险。</span></div></div></section><section class="section-grid-3"><article class="card"><div class="section-head"><h3>Uncovered</h3><span>没有明确后备</span></div><div class="priority-card"><strong>${safe(summary.kpis.uncovered)}</strong><p>这些岗位是直接的继任空窗。</p></div></article><article class="card"><div class="section-head"><h3>Weakly covered</h3><span>表面有覆盖，深度不够</span></div><div class="priority-card"><strong>${safe(Math.max(0, singlePoints.length - summary.kpis.uncovered))}</strong><p>这些岗位往往只依赖一个明显候选。</p></div></article><article class="card"><div class="section-head"><h3>Concentrated dependency</h3><span>过度绑定单个人</span></div><div class="priority-card"><strong>${safe(singlePoints.length)}</strong><p>覆盖数量不能代表可用深度。</p></div></article></section><section class="card"><div class="section-head"><h3>Coverage Snapshot</h3><span>每个指标都带判断</span></div><div class="grid-kpi">${coverageCards.map((item) => `<article class="card kpi-card"><span>${safe(item.label)}</span><strong>${safe(item.value)}</strong><small>${safe(item.note)}</small></article>`).join("")}</div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Single-point Dependency</h3><span>优先级高于热力图</span></div>${singlePoints.length ? singlePoints.map((role) => `<div class="priority-card"><strong>${safe(role.key)}</strong><p>${safe(role.candidates[0] ? `${role.candidates[0].name} 是当前唯一或明显领先候选人。` : "当前没有明确候选人。")}</p></div>`).join("") : `<div class="empty-state">当前未识别到显著单点依赖岗位。</div>`}</article><article class="card"><div class="section-head"><h3>Hidden Support-function Risk</h3><span>小部门风险最容易被忽视</span></div>${supportRoles.length ? supportRoles.map((role) => `<div class="priority-card"><strong>${safe(role.key)}</strong><p>${safe(role.candidates[0] ? `${role.candidates[0].name} 是当前最佳候选，准备度 ${readinessText(role.candidates[0].succession.band)}。` : "当前没有明确候选。")}</p></div>`).join("") : `<div class="empty-state">当前没有可展示的支持职能关键岗位。</div>`}</article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Role Risk Heatmap</h3><span>危险区证据</span></div><div class="heatmap-grid">${roles.slice(0, 9).map((role) => { const best = role.candidates[0]; const alpha = Math.max(0.2, Math.min(0.88, (best ? best.succession.score : 36) / 100)); return `<button class="heat-cell ${selectedRole && selectedRole.key === role.key ? "active" : ""}" data-role="${safe(role.key)}" style="background: rgba(47,107,255,${alpha})"><strong>${safe(role.department)}</strong><span>${safe(role.position)}</span><em>${safe(best ? readinessText(best.succession.band) : "暂无后备")}</em></button>`; }).join("")}</div></article><article class="card"><div class="section-head"><h3>Selected role</h3><span>${safe(selectedRole ? selectedRole.key : "暂无岗位")}</span></div><div class="priority-card"><strong>${safe(selectedRole && selectedRole.candidates[0] ? `${selectedRole.candidates[0].name} 是当前最优候选` : "当前没有可用后备")}</strong><p>${safe(selectedRole && selectedRole.candidates[0] ? `${readinessText(selectedRole.candidates[0].succession.band)}，综合分 ${selectedRole.candidates[0].succession.score}。` : "这类岗位属于立即需要命名后备的风险点。")}</p></div></article></section><section class="card"><div class="section-head"><h3>AI Succession Insights</h3><span>可直接用于汇报的继任判断</span></div>${topIssueCards(summary).filter((item) => /coverage|risk|dependency|bench|接班|岗位|继任|空窗/i.test(item.title + item.rootCause + item.managementRead)).slice(0, 4).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.phenomenon)}</p></div>`).join("")}</section>${renderProfileDrawer(selectedEmployee, "查看候选人画像")}`;
  }
  ...
  function renderReport() {
    ...
    const signals = predictSignals(summary);
    const issueCards = topIssueCards(summary);
    return `<section class="report-hero card"><div class="tag">Executive Report</div><h2>一页读懂当前状态、主要问题和优先动作</h2><p>这是收口页，不是图表收纳页。</p><div class="button-row"><button class="btn btn-primary" data-action="download-report">Export / Share</button><a class="btn btn-secondary" href="#overview">Back to Overview</a></div></section><section class="executive-summary card"><div class="executive-main"><div class="tag">Executive Report Summary</div><h2>${safe(summary.headline)}</h2><p>${safe(summary.priority)}</p><div class="summary-kpis"><div><span>Current state</span><strong>${safe(summary.state)}</strong></div><div><span>Risk level</span><strong>${safe(summary.riskLevel)}</strong></div></div></div><div class="executive-side"><div class="summary-point"><strong>Top issue</strong><span>${safe(issueCards[0] ? issueCards[0].title : summary.headline)}</span></div><div class="summary-point"><strong>Priority action</strong><span>${safe(actions.immediate[0] ? actions.immediate[0].action : summary.priority)}</span></div></div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Current State</h3><span>现在发生了什么</span></div><div class="priority-card"><strong>${safe(summary.state)}</strong></div></article><article class="card"><div class="section-head"><h3>Why It Is Happening</h3><span>真正的 root cause</span></div>${issueCards.slice(0, 2).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.rootCause)}</p></div>`).join("")}</article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>What May Happen Next</h3><span>如果不处理会怎样</span></div>${signals.slice(0, 4).map((text) => `<div class="priority-card"><strong>${safe(text)}</strong></div>`).join("")}</article><article class="card"><div class="section-head"><h3>What To Do Now</h3><span>现在该做什么</span></div>${actions.immediate.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(`${item.target} · ${item.purpose}`)}</p></div>`).join("")}</article></section><section class="card"><div class="section-head"><h3>Top Issues</h3><span>phenomenon / root cause / likely consequence / recommended move</span></div>${issueCards.map((item, index) => `<div class="priority-card"><strong>${safe(`${index + 1}. ${item.title}`)}</strong><p><b>phenomenon：</b>${safe(item.phenomenon)}</p><p><b>root cause：</b>${safe(item.rootCause)}</p><p><b>likely consequence：</b>${safe(item.likelyConsequence)}</p><p><b>recommended move：</b>${safe(item.judgment)}</p></div>`).join("")}</section><section class="section-grid-3"><article class="card"><div class="section-head"><h3>P1 Immediate risk reduction</h3><span>优先降风险</span></div>${actions.immediate.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(item.purpose)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>P2 Near-term stabilization</h3><span>近期稳定</span></div>${actions.mid.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(item.purpose)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>P3 Medium-term bench building</h3><span>中期补梯队</span></div>${actions.bench.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(item.purpose)}</p></div>`).join("")}</article></section>`;
  }
