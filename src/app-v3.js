(function () {
  ...
  const demo = window.TALENTPULSE_DEMO || { employees: [], metadata: {} };
  ...
  const TITLE_ALIAS = { PM: "Product Manager", "Cust Success Specialist": "Customer Success Consultant" };
  ...
  function normalizePotential(value) {
    const text = String(value || "").trim();
    if (/high/i.test(text) || text === "高") return "High";
    if (/medium/i.test(text) || text === "中") return "Medium";
    if (/low/i.test(text) || text === "低") return "Low";
    return "";
  }
  ...
  function enrichEmployees(rows) {
    ...
    rows.forEach((raw, index) => {
      ...
      const employee = {
        ...
        job_family: String(raw.job_family || "General").trim() || "General",
        ...
        flight_risk: String(raw.flight_risk || "Medium").trim() || "Medium",
  ...
  function detectQuality(rawRows, employees, mappingMeta) {
    ...
    const counts = {};
    rawRows.forEach((row) => {
      const id = String(row.employee_id || row.employeeId || row.工号 || "").trim();
      if (!id) return;
    ...
    rawRows.forEach((row, index) => {
      const line = index + 2;
      const id = String(row.employee_id || row.employeeId || row.工号 || `ROW-${line}`).trim();
      ...
      if (!String(row.potential_level || row.潜力 || "").trim() && scorePerformance(row.performance_current || row.当前绩效) >= 4) quality.caution.push(makeIssueItem("高绩效员工缺少潜力标签", line, id, "potential_level", "高绩效不应直接等同于高潜，建议补齐潜力判断。"));
  ...
  function summarizeDepartments(rows) {
    const summary = {};
    rows.forEach((row) => {
      if (!summary[row.department]) summary[row.department] = { count: 0, hipoA: 0, readyNow: 0, readySoon: 0, highRisk: 0, uncoveredRoles: 0, highPerf: 0, avgShl: 0 };
  ...
  function buildIssues(rows, summary) {
    ...
    Object.keys(summary).forEach((department) => {
      ...
      const hipoRate = Math.round((item.hipoA / item.count) * 100);
      const readyNowRate = Math.round((item.readyNow / item.count) * 100);
      const readySoonRate = Math.round((item.readySoon / item.count) * 100);
      const highRiskRate = Math.round((item.highRisk / item.count) * 100);

      if (department === "Engineering" && hipoRate >= 12 && readyNowRate <= 8) issues.push(makeInsight("研发高潜储备不错，但管理 ready now 偏薄", `研发高潜占比约 ${hipoRate}%，但现在可接任仅 ${readyNowRate}%。问题不是没人，而是高潜没有被及时转成梯队。`, 95, "review", "如果不推进带人历练与关键经验补齐，研发高潜会继续慢于业务增长完成转化。", "先核验研发潜在管理者名单并安排带人任务。", "把研发高潜纳入定向培养计划。", "形成研发梯队滚动盘点机制。"));
      if (department === "Sales" && highRiskRate >= 18 && item.highPerf >= 10) issues.push(makeInsight("销售结果强，但对头部人才依赖偏重", `销售高绩效密度高，但高离职风险约 ${highRiskRate}%。当前业绩更像被少数明星员工托住。`, 92, "succession", "一旦头部人才流失，销售岗位空缺暴露和业绩波动会被迅速放大。", "先对高绩效且高风险的人群做保留对话。", "补齐关键销售岗位的替补与跨区备份。", "降低组织对单点明星员工的结构性依赖。"));
      if (department === "Operations" && hipoRate <= 6 && readySoonRate <= 10) issues.push(makeInsight("运营团队稳定，但成长动能偏弱", `运营人数不少，但高潜占比约 ${hipoRate}%，1-2 年可接任占比约 ${readySoonRate}%。稳定不等于有后续增量。`, 84, "review", "如果继续沿用现有培养方式，运营会更像稳态支持团队，而不是可持续输出骨干的梯队池。", "先识别运营里的潜在骨干和跨岗培养对象。", "安排轮岗、专项改进项目和人才校准会议。", "把稳定团队转成可持续供给的后备梯队。"));
      if (department === "Product" && item.uncoveredRoles >= 2) issues.push(makeInsight("产品关键岗位覆盖偏窄，存在单点依赖", `产品至少有 ${item.uncoveredRoles} 个关键岗位缺少明确后备。岗位看起来稳定，但继任暴露并不低。`, 90, "succession", "如果出现核心岗位流动或组织调整，产品线恢复成本会显著上升。", "先核验产品关键岗位的后备名单。", "为高暴露岗位建立 ready soon 候选梯队。", "持续降低产品领导岗位的单点依赖。"));
      if (department === "Customer Success" && item.count >= 25 && hipoRate <= 7) issues.push(makeInsight("客户成功团队规模大，但高潜识别偏弱", `客户成功团队人数多，但高潜占比约 ${hipoRate}%。当前更像识别不足，而不是没有可培养对象。`, 82, "review", "如果继续低识别，未来管理和关键岗位供给会逐步不足。", "先复核客户成功团队的潜力识别口径。", "增加跨岗项目和服务管理历练机会。", "建立客户成功高潜识别与培养节奏。"));
      if (["HR", "Finance & Legal", "IT / Data Support"].includes(department) && item.uncoveredRoles >= 1) issues.push(makeInsight(`${department} 体量不大，但继任风险被低估`, `${department} 人数不多，却有 ${item.uncoveredRoles} 个关键岗位未形成明确覆盖。低可见度不等于低风险。`, 86, "succession", `如果支持职能关键岗位突然空缺，恢复速度会明显慢于业务岗位。`, `先核验 ${department} 关键岗位后备名单。`, `为 ${department} 补齐 ready soon 候选人和跨岗备份。`, `将支持职能纳入固定节奏的继任校准。`));
    });
    if (!issues.length) issues.push(makeInsight("组织需要优先核验关键岗位覆盖", "当前没有极端异常，但关键岗位覆盖和高潜转化仍值得优先确认。", 70, "succession", "如果不持续滚动复核，继任暴露会在组织调整时被放大。", "先核验高暴露岗位后备名单。", "建立 ready soon 候选池。", "形成周期性的继任盘点机制。"));
  ...
  function executiveSummary(rows) {
    ...
    const issues = buildIssues(rows, summary);
  ...
  function predictSignals(summary) {
    const signals = (summary.issues || []).map((item) => item.predict).filter(Boolean);
    if (summary.kpis.coverage < 70) signals.push("如果关键岗位覆盖率继续低于 70%，组织在扩张或人员流动阶段会更加被动。");
    if (summary.kpis.conversionGap >= 10) signals.push("如果高潜到 ready now 的缺口继续扩大，未来储备会难以转成短期可用的管理梯队。");
  ...
  function buildDemoStories(summary) {
    const engineering = departmentSnapshot(summary, "Engineering");
    const sales = departmentSnapshot(summary, "Sales");
    const operations = departmentSnapshot(summary, "Operations");
    const product = departmentSnapshot(summary, "Product");
    const supportRisk = ["HR", "Finance & Legal", "IT / Data Support"].map((name) => departmentSnapshot(summary, name)).reduce((acc, item) => ({
    ...
    return [
      {
        ...
        route: "review",
        title: "研发增长快于管理准备度",
        metric: `高潜 ${engineering.hipoA} / Ready Now ${engineering.readyNow}`,
        talk: `研发高潜储备并不差，但 ready now 偏薄，说明储备还没有顺利转成可用梯队。`,
      ...
      },
      {
        ...
        route: "review",
        title: "运营稳定，但成长动能偏弱",
        metric: `高潜 ${operations.hipoA} / Ready Soon ${operations.readySoon}`,
        talk: `运营团队稳定不代表健康，真正的问题是高潜和 ready soon 都偏少。`,
      ...
      },
      {
        ...
        why: "后备识别和 ready soon 候选池没有跟上岗位关键度。",
      ...
      },
      {
        ...
        metric: `缺后备 ${supportRisk.uncoveredRoles} / Ready Now ${supportRisk.readyNow}`,
        talk: `HR、财务和 IT 不显眼，但关键岗位一旦空缺，组织恢复速度会明显变慢。`,
        why: "低可见度岗位往往没有被放进固定节奏的继任校准里。",
        action: "把支持职能纳入固定盘点节奏，补齐 ready soon 候选。"
      }
    ];
  }
  ...
  function actionMeta(text) {
    ...
    if (/校准|滚动|机制/.test(text)) return { target: "业务负责人和 HRBP", purpose: "形成持续盘点节奏" };
  ...
  function topIssueCards(summary) {
    return summary.issues.slice(0, 4).map((item) => ({
      ...
      scope: item.route === "review" ? "人才结构与高潜转化" : item.route === "succession" ? "关键岗位与接班覆盖" : "组织健康",
      whyCare: item.predict || "如果不处理，当前问题会在业务扩张或人员变化时被放大。",
      judgment: item.actionNow || summary.priority
    }));
  }

  function departmentStoryCards(summary) {
    const stories = [
      { key: "Engineering", label: "R&D", insight: "Leadership readiness is lagging behind R&D growth" },
      { key: "Sales", label: "Sales", insight: "Sales strength is masking a fragile bench" },
      { key: "Operations", label: "Operations", insight: "Stable output is masking weak growth momentum" },
      { key: "Product", label: "Product", insight: "Product succession exposure is concentrated" },
      { key: "Customer Success", label: "Customer Success", insight: "Bench visibility is weaker than team size" }
    ];
    return stories.map((item) => {
      const dept = summary.summary[item.key] || { count: 0, hipoA: 0, readyNow: 0, highPerf: 0, uncoveredRoles: 0 };
      return {
        title: item.insight,
        detail: `${item.label} ${dept.count} 人，高潜 ${dept.hipoA}，Ready Now ${dept.readyNow}，无后备关键岗 ${dept.uncoveredRoles}。`
      };
    });
  }
  ...
  function benchGapCards(rows, summary) {
    ...
    const operations = summary.summary.Operations || { hipoA: 0, readySoon: 0 };
    return [
      { title: "High performance is stronger than bench maturity", detail: `${highPerfNotReady} 位高绩效员工尚未 ready-now，强结果没有自动转成可接班梯队。` },
      { title: "High potential concentration is not the same as management readiness", detail: `${highPotentialNotReady} 位高潜 A 还没有进入 ready-now，潜力转化慢于业务需求。` },
      { title: "Operational stability is not the same as long-term health", detail: `Operations 高潜 ${operations.hipoA}、ready soon ${operations.readySoon || 0}，稳定更多来自惯性而不是后备供给。` }
    ];
  }
  ...
  function supportRiskRoles(roles) {
    return roles.filter((role) => ["HR", "Finance & Legal", "IT / Data Support"].includes(role.department)).slice(0, 4);
  }
  ...
  function renderHome() {
    ...
    return `<section class="hero hero-v2 card home-hero"><div class="hero-copy"><div class="tag">AI Talent Review & Succession Diagnosis</div><h2>把员工数据转成可汇报的组织洞察</h2><p>面向 HR 学生与初级 HR 从业者的 AI 分析作品。它的重点不是配置流程，而是把杂乱员工数据变成管理层关心的问题、风险与动作。</p><div class="hero-actions"><button class="btn btn-primary" data-action="try-demo">Try Demo</button><label class="btn btn-secondary upload-inline">Upload Employee Data<input id="upload-input" type="file" accept=".csv,.xlsx" hidden></label></div><div class="hero-note">${safe(state.uploadNote)}</div></div><div class="preview-board"><div class="preview-board-head"><span>Result Preview</span><strong>${safe(preview.company)}</strong></div><div class="preview-score"><span>What you can show in 2-3 minutes</span><strong>组织状态、关键问题、风险外推、优先动作</strong></div><div class="preview-findings"><div class="preview-finding"><strong>AI reads messy HR data</strong><span>自动识别字段、统一口径、先给可信度再分析。</span></div><div class="preview-finding"><strong>Diagnosis before dashboards</strong><span>先讲判断，再展示证据、风险和动作。</span></div></div></div></section><section class="grid-kpi home-value-grid"><article class="card value-card"><span>Use AI to structure messy HR data</span><strong>自动识别常见 HR 字段并归一化口径</strong></article><article class="card value-card"><span>Surface hidden talent and bench risks</span><strong>把高潜断层、接班空窗和单点依赖显性化</strong></article><article class="card value-card"><span>Explain why strong teams can still be fragile</span><strong>把绩效强但梯队薄的机制讲清楚</strong></article><article class="card value-card"><span>Generate report-ready recommendations</span><strong>输出可直接汇报的管理语言和动作建议</strong></article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Demo Company Preview</h3><span>这不是普通 mock 数据</span></div><div class="story-grid"><div><strong>Company</strong><p>${safe(preview.company)}</p></div><div><strong>Industry</strong><p>${safe(preview.industry)}</p></div><div><strong>Employees</strong><p>${safe(`${preview.size} 人`)}</p></div><div><strong>Departments</strong><p>${safe(`${preview.departments} 个部门`)}</p></div></div><div class="issue-chip-grid push-top">${preview.issueTags.map((item) => `<span class="tag warning">${safe(item)}</span>`).join("")}</div></article><article class="card"><div class="section-head"><h3>How It Works</h3><span>极简主链路</span></div><div class="workflow-strip workflow-strip-v2"><div><span>Step 1</span><strong>Upload / Demo</strong></div><div><span>Step 2</span><strong>Auto-structure</strong></div><div><span>Step 3</span><strong>Diagnose</strong></div><div><span>Step 4</span><strong>Report</strong></div></div><div class="priority-card push-top"><strong>Upload reassurance</strong><p>支持 Excel / CSV，允许字段不统一，系统会先自动识别、清洗并给出分析可信度。</p></div></article></section>`;
  }
  ...
  function renderOverview() {
    ...
    const stateCards = [
      ...
      { label: "Ready Now", value: summary.kpis.readyNow, note: "可立即接任的人数仍然偏薄。" },
    ...
    return `<section class="executive-summary card"><div class="executive-main"><div class="tag">Executive Summary</div><h2>${safe(summary.headline)}</h2><p>${safe(summary.priority)}</p><div class="summary-kpis"><div><span>Overall judgment</span><strong>${safe(summary.state)}</strong></div><div><span>Immediate action</span><strong>${safe(summary.priority)}</strong></div></div></div><div class="executive-side">${summary.issues.slice(0, 4).map((item) => `<div class="summary-point"><strong>${safe(item.title)}</strong><span>${safe(item.detail)}</span></div>`).join("")}</div></section><section class="card"><div class="section-head"><h3>Current State</h3><span>先给证据，再解释怎么读</span></div><div class="grid-kpi">${stateCards.map((item) => `<article class="card kpi-card"><span>${safe(item.label)}</span><strong>${safe(item.value)}</strong><small>${safe(item.note)}</small></article>`).join("")}</div><div class="section-grid-2 push-top"><article class="card"><div class="section-head"><h3>Department difference</h3><span>最有故事的团队差异</span></div><div class="story-grid">${departmentStoryCards(summary).slice(0, 4).map((item) => `<div><strong>${safe(item.title)}</strong><p>${safe(item.detail)}</p></div>`).join("")}</div></article><article class="card"><div class="section-head"><h3>9-box signal</h3><span>九宫格只是证据，不是主角</span></div>${renderBars(ninebox.filter((cell) => cell.employees.length).sort((a, b) => b.employees.length - a.employees.length).slice(0, 4).map((cell) => ({ label: cell.title, value: cell.employees.length, color: cell.key === "3-3" ? "var(--primary)" : cell.key === "3-2" ? "#4a7cff" : "var(--warning)" })))}<div class="priority-card push-top"><strong>How to read</strong><p>右上格代表核心高潜，右中格代表“绩效强但接班未成熟”的关键人群。</p></div></article></div></section><section class="card"><div class="section-head"><h3>Key Issues</h3><span>固定只讲最重要的 4 个问题</span></div><div class="section-grid-2">${issueCards.map((item) => `<article class="card insight-card"><div class="insight-line"><strong>${safe(item.title)}</strong><p>${safe(item.phenomenon)}</p></div><div class="insight-line"><strong>Why it matters</strong><p>${safe(item.whyCare)}</p></div><div class="insight-line"><strong>Impact scope</strong><p>${safe(item.scope)}</p></div><div class="insight-line"><strong>Management judgment</strong><p>${safe(item.judgment)}</p></div></article>`).join("")}</div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Why It’s Happening</h3><span>解释机制，而不是重复描述</span></div>${benchGapCards(state.employees, summary).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.detail)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>Risk Outlook</h3><span>6-12 month outlook</span></div>${signals.slice(0, 4).map((text) => `<div class="priority-card"><strong>${safe(text)}</strong></div>`).join("")}</article></section><section class="card"><div class="section-head"><h3>Recommended Actions</h3><span>先降风险，再补梯队</span></div><div class="section-grid-3"><article class="card"><div class="section-head"><h3>Immediate</h3><span>0-3 months</span></div>${actions.immediate.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(`${item.target} · ${item.purpose}`)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>Mid-term</h3><span>3-6 months</span></div>${actions.mid.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(`${item.target} · ${item.purpose}`)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>Bench-building</h3><span>6-12 months</span></div>${actions.bench.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(`${item.target} · ${item.purpose}`)}</p></div>`).join("")}</article></div></section>${renderGuideCards([{ route: "review", label: "Talent Review", reason: "查看更多人才结构与伪强项证据" }, { route: "succession", label: "Succession", reason: "查看更多覆盖、空窗与单点依赖证据" }, { route: "health", label: "Data Health", reason: "查看 AI 如何理解并修复数据" }, { route: "report", label: "Report", reason: "进入完整汇报稿收口" }])}`;
  }
  function renderTalentReview() {
    ...
    const focusDepartments = ["Engineering", "Sales", "Operations", "Customer Success"].filter((department) => departments[department]);
    return `<section class="executive-summary card"><div class="executive-main"><div class="tag">Talent Snapshot</div><h2>${safe(summary.kpis.hipoA > summary.kpis.readyNow ? "Bench maturity is weaker than performance signals suggest" : "Talent density is usable, but not evenly converted into bench strength")}</h2><p>${safe(`当前样本 ${rows.length} 人。高潜 A ${summary.kpis.hipoA}，Ready Now ${summary.kpis.readyNow}，高绩效但未 ready-now ${countBy(rows, (row) => scorePerformance(row.performance_current) >= 4 && row.succession.band !== "Ready Now")} 人。`)}</p></div><div class="executive-side"><div class="summary-point"><strong>What this page proves</strong><span>人才密度、结构断层、伪强项。</span></div></div></section><section class="card"><div class="section-head"><h3>9-box Overview</h3><span>九宫格是证据，不是结论</span></div><div class="filter-row"><select class="filter-select" data-filter="department"><option value="All">全部部门</option>${unique(state.employees.map((row) => row.department)).map((department) => `<option value="${safe(department)}" ${state.selectedDepartment === department ? "selected" : ""}>${safe(department)}</option>`).join("")}</select><select class="filter-select" data-filter="jobLevel"><option value="All">全部职级</option>${unique(state.employees.map((row) => row.job_level)).map((level) => `<option value="${safe(level)}" ${state.selectedJobLevel === level ? "selected" : ""}>${safe(level)}</option>`).join("")}</select><span class="status info">当前样本 ${rows.length} 人</span></div><div class="ninebox-layout push-top"><div><div class="ninebox-grid">${cells.map((cell) => `<button class="ninebox-cell ${selectedCell && selectedCell.key === cell.key ? "active" : ""}" data-ninebox="${safe(cell.key)}"><span class="cell-title">${safe(cell.title)}</span><strong>${safe(cell.employees.length)}</strong><small>${safe(cell.key === "3-3" ? "高潜集中区" : cell.key === "3-2" ? "高绩效但梯队未成熟" : cell.key === "2-1" ? "稳定但成长有限" : "结构证据")}</small></button>`).join("")}</div><div class="priority-card push-top"><strong>${safe(selectedCell && selectedCell.key === "3-3" ? "高潜集中并不等于梯队成熟" : selectedCell && selectedCell.key === "3-2" ? "高绩效集中区暴露出接班不足" : "当前格子更适合说明结构差异")}</strong><p>${safe(selectedCell && selectedCell.employees.length ? `当前格子 ${selectedCell.employees.length} 人。` : "当前格子暂无代表员工。")}</p></div></div><div><div class="section-head"><h3>Linked employees</h3><span>${safe(selectedCell ? selectedCell.title : "暂无")}</span></div><div class="list-panel">${cellEmployees.length ? cellEmployees.slice(0, 10).map((employee) => `<button class="employee-row ${selectedEmployee && selectedEmployee.employee_id === employee.employee_id ? "active" : ""}" data-employee="${safe(employee.employee_id)}"><strong>${safe(employee.name)}</strong><span>${safe(employee.department + " / " + employee.position_title + " / " + readinessText(employee.succession.band))}</span></button>`).join("") : `<div class="empty-state">当前格子没有代表员工。</div>`}</div></div></div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Department Comparison</h3><span>只讲最有故事的 4 个部门</span></div><div class="story-grid">${focusDepartments.map((department) => `<div><strong>${safe(department)}</strong><p>${safe(`${departments[department].count} 人，高潜 ${departments[department].hipoA}，Ready Now ${departments[department].readyNow}，高绩效 ${departments[department].highPerf}。`)}</p></div>`).join("")}</div></article><article class="card"><div class="section-head"><h3>Misleading Strengths</h3><span>伪强项比弱项更危险</span></div>${misleading.map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.detail)}</p></div>`).join("")}</article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Bench Gaps</h3><span>哪里存在梯队断层</span></div>${topIssueCards(summary).filter((item) => /人才|高潜|梯队/.test(item.scope + item.title)).slice(0, 3).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.phenomenon)}</p></div>`).join("")}</article>${renderProfileCard(selectedEmployee, "人才样本", true)}</section><section class="card"><div class="section-head"><h3>AI Talent Insights</h3><span>可直接拿来讲的判断</span></div>${departmentStoryCards(summary).slice(0, 4).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.detail)}</p></div>`).join("")}</section>`;
  }
  ...
  function renderSuccession() {
    ...
    const coverageCards = [
      { label: "Critical roles", value: summary.kpis.criticalRoles, note: "关键岗位数量决定组织的真实接班压力。" },
      { label: "Covered roles", value: summary.kpis.coveredCriticalRoles, note: "被提名不等于可用，覆盖只是第一层。" },
      { label: "Ready Now", value: summary.kpis.readyNow, note: "真正能马上接上的人数依然有限。" },
      { label: "Ready Soon", value: countBy(state.employees, (row) => row.succession.band === "Ready in 1-2 Years"), note: "这是 1-2 年内最值得培养的梯队池。" },
      { label: "Uncovered roles", value: summary.kpis.uncovered, note: "这些岗位是最直接的继任空窗。" }
    ];
    return `<section class="executive-summary card"><div class="executive-main"><div class="tag">Succession Health Summary</div><h2>${safe(summary.kpis.coverage < 70 ? "Coverage is visible, but bench depth is still shallow" : "Coverage is usable, but too many roles still rely on one candidate")}</h2><p>${safe(`关键岗位 ${summary.kpis.criticalRoles} 个，已覆盖 ${summary.kpis.coveredCriticalRoles} 个，Ready Now ${summary.kpis.readyNow} 人。`)}</p></div><div class="executive-side"><div class="summary-point"><strong>What this page proves</strong><span>覆盖、空窗、单点依赖、支持部门风险。</span></div></div></section><section class="card"><div class="section-head"><h3>Coverage Snapshot</h3><span>每个指标都带判断</span></div><div class="grid-kpi">${coverageCards.map((item) => `<article class="card kpi-card"><span>${safe(item.label)}</span><strong>${safe(item.value)}</strong><small>${safe(item.note)}</small></article>`).join("")}</div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Role Risk Heatmap</h3><span>先看危险区，再下钻岗位</span></div><div class="heatmap-grid">${roles.slice(0, 9).map((role) => { const best = role.candidates[0]; const alpha = Math.max(0.2, Math.min(0.88, (best ? best.succession.score : 36) / 100)); return `<button class="heat-cell ${selectedRole && selectedRole.key === role.key ? "active" : ""}" data-role="${safe(role.key)}" style="background: rgba(47,107,255,${alpha})"><strong>${safe(role.department)}</strong><span>${safe(role.position)}</span><em>${safe(best ? readinessText(best.succession.band) : "暂无后备")}</em></button>`; }).join("")}</div></article><article class="card"><div class="section-head"><h3>Selected role</h3><span>${safe(selectedRole ? selectedRole.key : "暂无岗位")}</span></div><div class="priority-card"><strong>${safe(selectedRole && selectedRole.candidates[0] ? `${selectedRole.candidates[0].name} 是当前最优候选` : "当前没有可用后备")}</strong><p>${safe(selectedRole && selectedRole.candidates[0] ? `${readinessText(selectedRole.candidates[0].succession.band)}，综合分 ${selectedRole.candidates[0].succession.score}。` : "这类岗位属于立即需要命名后备的风险点。")}</p></div><div class="list-panel">${selectedRole ? selectedRole.candidates.map((employee) => `<button class="employee-row ${selectedEmployee && selectedEmployee.employee_id === employee.employee_id ? "active" : ""}" data-employee="${safe(employee.employee_id)}"><strong>${safe(employee.name)}</strong><span>${safe(readinessText(employee.succession.band) + " / " + employee.succession.score)}</span></button>`).join("") : `<div class="empty-state">当前没有可展示的关键岗位。</div>`}</div></article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Single-point Dependency</h3><span>表面有覆盖，实则过度依赖一个人</span></div>${singlePoints.length ? singlePoints.map((role) => `<div class="priority-card"><strong>${safe(role.key)}</strong><p>${safe(role.candidates[0] ? `${role.candidates[0].name} 是当前唯一或明显领先候选人。` : "当前没有明确候选人。")}</p></div>`).join("") : `<div class="empty-state">当前未识别到显著单点依赖岗位。</div>`}</article><article class="card"><div class="section-head"><h3>Hidden Support-function Risk</h3><span>小部门风险最容易被忽视</span></div>${supportRoles.length ? supportRoles.map((role) => `<div class="priority-card"><strong>${safe(role.key)}</strong><p>${safe(role.candidates[0] ? `${role.candidates[0].name} 是当前最佳候选，准备度 ${readinessText(role.candidates[0].succession.band)}。` : "当前没有明确候选。")}</p></div>`).join("") : `<div class="empty-state">当前没有可展示的支持职能关键岗位。</div>`}</article></section><section class="section-grid-2">${renderProfileCard(selectedEmployee, "候选人画像", true)}<article class="card"><div class="section-head"><h3>AI Succession Insights</h3><span>可直接用于汇报的继任判断</span></div>${topIssueCards(summary).filter((item) => /接班|岗位|继任/.test(item.title + item.scope)).slice(0, 4).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.phenomenon)}</p></div>`).join("")}</article></section>`;
  }
  ...
  function renderHealth() {
    ...
    return `<section class="executive-summary card"><div class="executive-main"><div class="tag">Readiness Summary</div><h2>${safe(quality.confidence.score >= 82 ? "Analysis confidence is strong enough for diagnosis" : quality.confidence.score >= 68 ? "Analysis is usable, but some assumptions need caution" : "Data confidence is limiting the strength of conclusions")}</h2><p>${safe(`已识别 ${state.mappingMeta.matchedFields.length} 个分析字段，低置信字段 ${state.mappingMeta.lowConfidenceFields.length} 个，关键风险 ${quality.risk.length} 项。`)}</p></div><div class="executive-side"><div class="summary-point"><strong>Analysis confidence</strong><span>${safe(`${quality.confidence.score} / ${quality.confidence.label}`)}</span></div><div class="summary-point"><strong>What it affects</strong><span>${safe(quality.risk.length ? "继任判断和组织结构解释需要带着风险提示看。" : "当前结果足以直接进入组织诊断和汇报。")}</span></div></div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Auto-understood Fields</h3><span>AI 自动理解了什么</span></div><div class="story-grid"><div><strong>Matched fields</strong><p>${safe(`${state.mappingMeta.matchedFields.length} 个字段进入主分析链路`)}</p></div><div><strong>Low-confidence</strong><p>${safe(`${state.mappingMeta.lowConfidenceFields.length} 个字段建议人工复核`)}</p></div><div><strong>Unused source fields</strong><p>${safe(`${state.mappingMeta.unmappedHeaders.length} 个字段未影响当前分析`)}</p></div><div><strong>Current recommendation</strong><p>${safe(quality.confidence.label === "高" ? "可以直接用于演示和汇报" : "适合继续分析，但要带着风险提示解释")}</p></div></div></article><article class="card"><div class="section-head"><h3>Normalization Actions</h3><span>AI 自动做了什么</span></div>${normalizationActions().map((item) => `<div class="priority-card"><strong>${safe(item)}</strong></div>`).join("")}</article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Low-confidence Areas</h3><span>诚实指出不稳的地方</span></div>${(state.mappingMeta.lowConfidenceFields.length ? state.mappingMeta.lowConfidenceFields : ["当前低置信字段较少"]).slice(0, 6).map((field) => `<div class="priority-card"><strong>${safe(field)}</strong><p>${safe(field === "当前低置信字段较少" ? "当前自动识别结果较稳定。" : "该字段通过相似命名推断得到，建议人工确认口径。")}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>Issue Evidence</h3><span>保留最关键的问题样本</span></div><div class="tabs"><button class="tab ${state.activeHealthTab === "autoFixed" ? "active" : ""}" data-health-tab="autoFixed">已自动修复</button><button class="tab ${state.activeHealthTab === "caution" ? "active" : ""}" data-health-tab="caution">需谨慎解读</button><button class="tab ${state.activeHealthTab === "risk" ? "active" : ""}" data-health-tab="risk">关键风险</button></div>${selectedIssue ? `<div class="priority-card"><strong>${safe(selectedIssue.title)}</strong><p>${safe(selectedIssue.detail)}</p></div>` : `<div class="empty-state">当前分类暂无问题。</div>`}<table class="data-table compact"><thead><tr><th>问题</th><th>字段</th><th>员工</th></tr></thead><tbody>${rows.length ? rows.slice(0, 6).map((item, index) => `<tr class="${state.selectedHealthIndex === index ? "table-row-active" : ""}" data-health-index="${index}"><td>${safe(item.title)}</td><td>${safe(item.field)}</td><td>${safe(item.employeeId)}</td></tr>`).join("") : `<tr><td colspan="3"><div class="empty-state compact-empty">当前分类暂无问题。</div></td></tr>`}</tbody></table></article></section><section class="card"><div class="section-head"><h3>Analysis Confidence</h3><span>会影响什么，不影响什么</span></div><div class="grid-kpi"><article class="card kpi-card"><span>已自动修复</span><strong>${quality.autoFixed.length}</strong><small>这些问题已不再阻断主分析链路。</small></article><article class="card kpi-card"><span>需谨慎解读</span><strong>${quality.caution.length}</strong><small>这些问题会影响高潜或部门对比的精度。</small></article><article class="card kpi-card"><span>关键风险</span><strong>${quality.risk.length}</strong><small>这些问题会直接影响继任判断与汇报结论。</small></article></div></section>`;
  }
  ...
  function renderReport() {
    ...
    const signals = predictSignals(summary);
    const issueCards = topIssueCards(summary);
    return `<section class="report-hero card"><div class="tag">Executive Report</div><h2>一页读懂当前状态、主要问题和优先动作</h2><p>这是收口页，不是图表收纳页。</p><div class="button-row"><button class="btn btn-primary" data-action="download-report">Export / Share</button><a class="btn btn-secondary" href="#overview">Back to Overview</a></div></section><section class="executive-summary card"><div class="executive-main"><div class="tag">Executive Report Summary</div><h2>${safe(summary.headline)}</h2><p>${safe(summary.priority)}</p><div class="summary-kpis"><div><span>Current state</span><strong>${safe(summary.state)}</strong></div><div><span>Risk level</span><strong>${safe(summary.riskLevel)}</strong></div></div></div><div class="executive-side"><div class="summary-point"><strong>Top issue</strong><span>${safe(issueCards[0] ? issueCards[0].title : summary.headline)}</span></div><div class="summary-point"><strong>Priority action</strong><span>${safe(actions.immediate[0] ? actions.immediate[0].action : summary.priority)}</span></div></div></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>Current State</h3><span>现在发生了什么</span></div><div class="priority-card"><strong>${safe(summary.state)}</strong></div></article><article class="card"><div class="section-head"><h3>Why It Is Happening</h3><span>为什么会这样</span></div>${issueCards.slice(0, 2).map((item) => `<div class="priority-card"><strong>${safe(item.title)}</strong><p>${safe(item.phenomenon)}</p></div>`).join("")}</article></section><section class="section-grid-2"><article class="card"><div class="section-head"><h3>What May Happen Next</h3><span>如果不处理会怎样</span></div>${signals.slice(0, 4).map((text) => `<div class="priority-card"><strong>${safe(text)}</strong></div>`).join("")}</article><article class="card"><div class="section-head"><h3>What To Do Now</h3><span>现在该做什么</span></div>${actions.immediate.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(`${item.target} · ${item.purpose}`)}</p></div>`).join("")}</article></section><section class="card"><div class="section-head"><h3>Top Issues</h3><span>现象、原因、风险、建议</span></div>${issueCards.map((item, index) => `<div class="priority-card"><strong>${safe(`${index + 1}. ${item.title}`)}</strong><p><b>现象：</b>${safe(item.phenomenon)}</p><p><b>原因：</b>${safe(item.scope)}</p><p><b>风险：</b>${safe(item.whyCare)}</p><p><b>建议：</b>${safe(item.judgment)}</p></div>`).join("")}</section><section class="section-grid-3"><article class="card"><div class="section-head"><h3>P1 Immediate risk reduction</h3><span>优先降风险</span></div>${actions.immediate.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(item.purpose)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>P2 Near-term stabilization</h3><span>近期稳定</span></div>${actions.mid.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(item.purpose)}</p></div>`).join("")}</article><article class="card"><div class="section-head"><h3>P3 Medium-term bench building</h3><span>中期补梯队</span></div>${actions.bench.map((item) => `<div class="priority-card"><strong>${safe(item.action)}</strong><p>${safe(item.purpose)}</p></div>`).join("")}</article></section>`;
  }
  function renderShell() {
    ...
    return `<div class="shell"><aside class="sidebar"><div class="brand"><div class="brand-mark">TP</div><div><div class="brand-title">TalentPulse</div><div class="brand-sub">AI 人才盘点与继任诊断作品</div></div></div><nav class="nav">${visibleRoutes().map((route) => `<a class="nav-item ${state.route === route.key ? "active" : ""}" href="#${route.key}">${safe(route.label)}</a>`).join("")}</nav><div class="sidebar-foot"><div class="small-label">数据来源</div><div class="sidebar-company">${safe(state.sourceName)}</div></div></aside><section class="main"><header class="topbar"><div><div class="page-eyebrow">结论 → 证据 → 解释 → 风险 → 动作</div><h1>${safe(routeLabel(state.route))}</h1></div><div class="topbar-actions"><button class="btn btn-secondary" data-action="try-demo">Try Demo</button><label class="btn btn-primary upload-inline">Upload Employee Data<input id="upload-input-top" type="file" accept=".csv,.xlsx" hidden></label></div></header><main class="content">${content}</main></section></div>`;
  }
  ...
  function handleDemo() {
    useRows(demo.employees || [], "官方 Demo", "demo");
    ...
    state.uploadNote = "已切回官方 Demo，可直接进入总览开始讲述。";
  ...
  function useUploadedRows(rows, filename) {
    if (!rows || !rows.length) {
      state.uploadNote = "未识别到可用数据，请检查表头、分隔符，或先体验官方 Demo。";
      state.route = "home";
  ...
  function downloadReport() {
    const summary = executiveSummary(state.employees);
    const actions = actionPlan(summary);
    const content = ["TalentPulse 汇报摘要", "", "一、当前状态", summary.state, "", "二、重点问题", summary.issues.map((item) => `- ${item.title}：${item.detail}`).join("\n"), "", "三、未来风险", predictSignals(summary).map((item) => `- ${item}`).join("\n"), "", "四、优先动作", `- 0-3 个月：${actions.now.join(" ")}`, `- 3-6 个月：${actions.soon.join(" ")}`, `- 6-12 个月：${actions.later.join(" ")}`].join("\n");
