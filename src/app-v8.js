(function () {
  ...
  const demo = window.TALENTPULSE_DEMO_V8 || { clean: { employee_master: [], workforce_monthly: [], recruiting_reqs: [], exit_events: [], critical_roles: [], metadata: {} }, dirty: { employee_master: [], workforce_monthly: [], recruiting_reqs: [], exit_events: [], critical_roles: [], metadata: {} }, metadata: {} };
  const analysisEngine = window.TalentPulseAnalysisV8 || null;
  ...
  const DEPT_ALIAS = {
    ...
    "Engineering": "研发中心",
    "Marketing": "市场品牌",
    "CEO Office / Strategy Office": "战略与经营"
  };
  ...
  const WORKFORCE_FIELDS = ["month", "department", "company_headcount", "department_headcount", "hires", "exits", "voluntary_attrition_rate", "regrettable_attrition_rate", "internal_moves", "ready_now_coverage", "ready_soon_coverage", "high_potential_rate", "critical_role_gap_count"];
  const WORKFORCE_ALIASES = {
    month: ["month", "月份", "统计月份", "快照月份"],
    department: ["department", "dept", "部门"],
    company_headcount: ["companyheadcount", "headcount", "公司人数", "公司月末人数", "总人数"],
    department_headcount: ["departmentheadcount", "deptheadcount", "部门人数", "部门月末人数"],
    hires: ["hires", "入职数", "招聘到岗数"],
    exits: ["exits", "离职数", "离开人数"],
    voluntary_attrition_rate: ["voluntaryattritionrate", "自愿离职率", "主动离职率"],
    regrettable_attrition_rate: ["regrettableattritionrate", "关键人才流失率", "遗憾离职率"],
    internal_moves: ["internalmoves", "内部流动数", "内部调动数"],
    ready_now_coverage: ["readynowcoverage", "readynowcoverage", "现在可接任覆盖率", "近端接班覆盖率"],
    ready_soon_coverage: ["readysooncoverage", "1-2年可接任覆盖率", "readysoon"],
    high_potential_rate: ["highpotentialrate", "高潜比例", "高潜识别率"],
    critical_role_gap_count: ["criticalrolegapcount", "关键岗位空缺数", "关键岗位空窗数", "关键岗位缺口数"]
  };

  const RECRUITING_FIELDS = ["req_id", "department", "position", "critical_role_flag", "open_date", "close_date", "time_to_fill_days", "source_channel", "recruiter_type", "agency_cost", "total_cost", "offer_count", "offer_acceptance_rate", "internal_fill_flag"];
  const RECRUITING_ALIASES = {
    req_id: ["reqid", "招聘需求id", "需求编号", "岗位需求编号"],
    department: ["department", "dept", "部门"],
    position: ["position", "岗位", "职位", "招聘岗位"],
    critical_role_flag: ["criticalroleflag", "关键岗位", "关键岗位标记"],
    open_date: ["opendate", "开放日期", "需求开启日期", "发布日期"],
    close_date: ["closedate", "关闭日期", "需求关闭日期", "到岗日期"],
    time_to_fill_days: ["timetofilldays", "招聘周期", "招聘天数", "到岗天数"],
    source_channel: ["sourcechannel", "招聘渠道", "来源渠道"],
    recruiter_type: ["recruitertype", "招聘类型", "招聘负责人类型"],
    agency_cost: ["agencycost", "猎头费用", "渠道费用"],
    total_cost: ["totalcost", "总招聘成本", "招聘成本"],
    offer_count: ["offercount", "offer数", "offer数量"],
    offer_acceptance_rate: ["offeracceptancerate", "offer接受率", "录用接受率"],
    internal_fill_flag: ["internalfillflag", "内部补位", "内部补位标记"]
  };

  const EXIT_FIELDS = ["exit_id", "employee_id", "exit_date", "department", "level", "regrettable_flag", "reason_tag", "replacement_time_days", "impact_level"];
  const EXIT_ALIASES = {
    exit_id: ["exitid", "离职事件id", "离职编号"],
    employee_id: ["employeeid", "employee_id", "工号", "员工编号"],
    exit_date: ["exitdate", "离职日期"],
    department: ["department", "dept", "部门"],
    level: ["level", "joblevel", "职级"],
    regrettable_flag: ["regrettableflag", "关键流失标记", "遗憾离职标记"],
    reason_tag: ["reasontag", "离职原因标签", "离职原因"],
    replacement_time_days: ["replacementtimedays", "替补到岗天数", "替补周期"],
    impact_level: ["impactlevel", "影响等级", "影响程度"]
  };

  const ROLE_FIELDS = ["role_id", "department", "position", "incumbent_id", "critical_role_flag", "critical_gap", "succession_depth", "risk_source_tags"];
  const ROLE_ALIASES = {
    role_id: ["roleid", "岗位编号", "关键岗位id"],
    department: ["department", "dept", "部门"],
    position: ["position", "岗位", "职位"],
    incumbent_id: ["incumbentid", "当前任职人工号", "在岗人工号"],
    critical_role_flag: ["criticalroleflag", "关键岗位", "关键岗位标记"],
    critical_gap: ["criticalgap", "关键经验缺口", "关键岗位缺口"],
    succession_depth: ["successiondepth", "后备深度", "继任覆盖深度"],
    risk_source_tags: ["risksourcetags", "风险标签", "风险来源标签"]
  };

  const TABLE_SCHEMAS = [
    { key: "employee_master", label: "员工表", model: "EmployeeCurrent", fields: FIELDS, aliases: ALIASES, required: ["employee_id", "name", "department", "position_title"], hints: ["employee", "员工", "花名册", "员工表", "人员"] },
    { key: "workforce_monthly", label: "月度快照", model: "WorkforceMonthly", fields: WORKFORCE_FIELDS, aliases: WORKFORCE_ALIASES, required: ["month", "department"], hints: ["monthly", "snapshot", "月度", "快照", "趋势"] },
    { key: "recruiting_reqs", label: "招聘表", model: "RecruitingReq", fields: RECRUITING_FIELDS, aliases: RECRUITING_ALIASES, required: ["req_id", "department", "position"], hints: ["recruit", "req", "招聘", "需求", "hc"] },
    { key: "exit_events", label: "离职表", model: "ExitEvent", fields: EXIT_FIELDS, aliases: EXIT_ALIASES, required: ["exit_id", "employee_id", "exit_date"], hints: ["exit", "attrition", "离职", "流失"] },
    { key: "critical_roles", label: "关键岗位表", model: "CriticalRole", fields: ROLE_FIELDS, aliases: ROLE_ALIASES, required: ["department", "position"], hints: ["critical", "role", "关键岗位", "岗位暴露"] }
  ];
  ...
  const state = {
    ...
    ingestion: { sourceFiles: [], detectedTables: [], translatedModels: [], analysisDepth: { key: "full", label: "完整分析", detail: "" }, verdict: { label: "可以直接演示", detail: "", tone: "safe" }, impacts: [], canAnalyze: true },
  ...
  function genericAutoMapRows(rawRows, schema) {
    const headers = rawRows.length ? Object.keys(rawRows[0]) : [];
    const normalized = headers.map((header) => ({ raw: header, normalized: normalizeKey(header) }));
    const fieldMap = {};
    const matchedFields = [];
    const lowConfidenceFields = [];
    const approximateFields = [];

    (schema.fields || []).forEach((field) => {
      const aliases = ((schema.aliases && schema.aliases[field]) || []).map(normalizeKey);
      ...
      if (lowConfidence) approximateFields.push(field);
      if (!match || lowConfidence) lowConfidenceFields.push(field);
    });

    const rows = rawRows.map((raw) => {
      const mapped = {};
      (schema.fields || []).forEach((field) => { mapped[field] = fieldMap[field] ? raw[fieldMap[field]] : ""; });
      return mapped;
    });

    const used = unique(Object.values(fieldMap));
    const missingRequiredFields = (schema.required || []).filter((field) => !fieldMap[field]);
    const unmappedHeaders = headers.filter((header) => !used.includes(header));
    return { fieldMap, matchedFields, lowConfidenceFields, approximateFields, missingRequiredFields, unmappedHeaders, rows };
  }

  function detectTableType(rawRows, sheetName) {
    const candidates = TABLE_SCHEMAS.map((schema) => {
      const mapping = genericAutoMapRows(rawRows, schema);
      const requiredHits = (schema.required || []).filter((field) => mapping.fieldMap[field]).length;
      const exactHits = mapping.matchedFields.length - mapping.approximateFields.length;
      const hintBonus = (schema.hints || []).some((hint) => normalizeKey(sheetName).includes(normalizeKey(hint))) ? 3 : 0;
      const sample = mapping.rows[0] || {};
      let rowBonus = 0;
      if (schema.key === "workforce_monthly" && (normalizeMonth(sample.month) || toNumber(sample.department_headcount, 0) || toNumber(sample.company_headcount, 0))) rowBonus += 2;
      if (schema.key === "recruiting_reqs" && (normalizeDate(sample.open_date) || toNumber(sample.time_to_fill_days, 0) || sample.req_id)) rowBonus += 2;
      if (schema.key === "exit_events" && (normalizeDate(sample.exit_date) || sample.reason_tag)) rowBonus += 2;
      if (schema.key === "critical_roles" && (sample.position || sample.department)) rowBonus += 1;
      const score = requiredHits * 5 + exactHits * 2 + mapping.matchedFields.length + hintBonus + rowBonus;
      return { schema, mapping, score, requiredHits };
    }).sort((a, b) => b.score - a.score);

    const best = candidates[0];
    const confidence = best ? clamp(Math.round((best.score / Math.max(8, (best.schema.fields.length * 2))) * 100), 0, 99) : 0;
    if (!best || best.requiredHits === 0 || best.score < 8) {
      return { key: "unknown", label: "未识别表", model: "-", confidence: 0, mapping: { fieldMap: {}, matchedFields: [], lowConfidenceFields: [], approximateFields: [], missingRequiredFields: [], unmappedHeaders: Object.keys(rawRows[0] || {}), rows: rawRows } };
    }
    return { key: best.schema.key, label: best.schema.label, model: best.schema.model, confidence, mapping: best.mapping };
  }
  ...
  function hasEmployeeData() {
    return state.employees.length > 0;
  }

  function hasOperationalData() {
    return state.workforceMonthly.length > 0 || state.recruitingReqs.length > 0 || state.exitEvents.length > 0;
  }

  function currentAnalysisDepth() {
    if (!hasEmployeeData()) return { key: "incomplete", label: "待补充", detail: "缺少员工表，暂不能进入分析结果。", canAnalyze: false, hasOperations: false };
    if (state.workforceMonthly.length > 0 || state.exitEvents.length > 0) return { key: "full", label: "完整分析", detail: "已具备人员、招聘和组织演化分析。", canAnalyze: true, hasOperations: true };
    if (state.recruitingReqs.length > 0) return { key: "enhanced", label: "增强分析", detail: "已增加招聘周期、成本与招聘效率分析。", canAnalyze: true, hasOperations: true };
    return { key: "basic", label: "基础分析", detail: "当前基于员工表输出现状、人才盘点与基础继任。", canAnalyze: true, hasOperations: false };
  }

  function visibleRoutes() {
    const depth = currentAnalysisDepth();
    return ROUTES.filter((route) => {
      if (route.key === "operations") return depth.hasOperations;
      if (["overview", "review", "succession", "report"].includes(route.key)) return depth.canAnalyze;
      return true;
    });
  }
  ...
  function translateWorkforceMonthly(rows) {
    return (rows || []).map((raw) => ({
      month: normalizeMonth(raw.month),
      department: normalizeDepartmentName(raw.department),
      company_headcount: Math.max(0, toNumber(raw.company_headcount, 0)),
      department_headcount: Math.max(0, toNumber(raw.department_headcount, 0)),
      hires: Math.max(0, toNumber(raw.hires, 0)),
      exits: Math.max(0, toNumber(raw.exits, 0)),
      voluntary_attrition_rate: clamp(toNumber(raw.voluntary_attrition_rate, 0), 0, 40),
      regrettable_attrition_rate: clamp(toNumber(raw.regrettable_attrition_rate, 0), 0, 20),
      internal_moves: Math.max(0, toNumber(raw.internal_moves, 0)),
      ready_now_coverage: clamp(toNumber(raw.ready_now_coverage, 0), 0, 100),
      ready_soon_coverage: clamp(toNumber(raw.ready_soon_coverage, 0), 0, 100),
      high_potential_rate: clamp(toNumber(raw.high_potential_rate, 0), 0, 100),
      critical_role_gap_count: Math.max(0, toNumber(raw.critical_role_gap_count, 0))
    })).filter((row) => row.month && row.department);
  }

  function translateRecruitingReqs(rows) {
    return (rows || []).map((raw, index) => ({
      req_id: String(raw.req_id || `REQ-UP-${index + 1}`).trim(),
      department: normalizeDepartmentName(raw.department),
      position: TITLE_ALIAS[String(raw.position || "").trim()] || String(raw.position || "").trim(),
      critical_role_flag: normalizeFlag(raw.critical_role_flag || "N"),
      open_date: normalizeDate(raw.open_date),
      close_date: normalizeDate(raw.close_date),
      time_to_fill_days: Math.max(0, toNumber(raw.time_to_fill_days, 0)),
      source_channel: String(raw.source_channel || "直招").trim() || "直招",
      recruiter_type: String(raw.recruiter_type || "HR").trim() || "HR",
      agency_cost: Math.max(0, toNumber(raw.agency_cost, 0)),
      total_cost: Math.max(0, toNumber(raw.total_cost, 0)),
      offer_count: Math.max(0, toNumber(raw.offer_count, 0)),
      offer_acceptance_rate: clamp(toNumber(raw.offer_acceptance_rate, 0), 0, 100),
      internal_fill_flag: normalizeFlag(raw.internal_fill_flag || "N")
    })).filter((row) => row.department && row.position);
  }

  function translateExitEvents(rows) {
    return (rows || []).map((raw, index) => ({
      exit_id: String(raw.exit_id || `EXIT-UP-${index + 1}`).trim(),
      employee_id: String(raw.employee_id || "").trim(),
      exit_date: normalizeDate(raw.exit_date),
      department: normalizeDepartmentName(raw.department),
      level: String(raw.level || "").trim(),
      regrettable_flag: normalizeFlag(raw.regrettable_flag || "N"),
      reason_tag: String(raw.reason_tag || "未标注").trim() || "未标注",
      replacement_time_days: Math.max(0, toNumber(raw.replacement_time_days, 0)),
      impact_level: normalizeImpactLevel(raw.impact_level)
    })).filter((row) => row.exit_date && (row.employee_id || row.department));
  }

  function translateCriticalRoles(rows) {
    return (rows || []).map((raw, index) => ({
      role_id: String(raw.role_id || `CR-UP-${index + 1}`).trim(),
      department: normalizeDepartmentName(raw.department),
      position: TITLE_ALIAS[String(raw.position || "").trim()] || String(raw.position || "").trim(),
      incumbent_id: String(raw.incumbent_id || "").trim(),
      critical_role_flag: normalizeFlag(raw.critical_role_flag || "Y"),
      critical_gap: String(raw.critical_gap || "Medium").trim() || "Medium",
      succession_depth: Math.max(0, toNumber(raw.succession_depth, 0)),
      risk_source_tags: String(raw.risk_source_tags || "").trim()
    })).filter((row) => row.department && row.position);
  }
  ...
  function depthFromData(data) {
    const hasEmployees = (data.employees || []).length > 0;
    const hasRecruiting = (data.recruitingReqs || []).length > 0;
    const hasHistory = (data.workforceMonthly || []).length > 0 || (data.exitEvents || []).length > 0;
    if (!hasEmployees) return { key: "incomplete", label: "待补充", detail: "缺少员工表，暂不能进入分析结果。", canAnalyze: false, hasOperations: false };
    if (hasHistory) return { key: "full", label: "完整分析", detail: "已具备三年趋势与组织演化分析。", canAnalyze: true, hasOperations: true };
    if (hasRecruiting) return { key: "enhanced", label: "增强分析", detail: "已增加招聘周期、招聘成本与招聘效率分析。", canAnalyze: true, hasOperations: true };
    return { key: "basic", label: "基础分析", detail: "当前仅基于员工表输出现状、人才盘点与基础继任。", canAnalyze: true, hasOperations: false };
  }

  function mergeEmployeeMapping(tables) {
    const employeeTables = (tables || []).filter((item) => item.key === "employee_master");
    if (!employeeTables.length) return { fieldMap: {}, matchedFields: [], lowConfidenceFields: [], unmappedHeaders: [], rows: [] };
    return {
      fieldMap: employeeTables[0].mapping.fieldMap,
      matchedFields: unique(employeeTables.flatMap((item) => item.mapping.matchedFields || [])),
      lowConfidenceFields: unique(employeeTables.flatMap((item) => item.mapping.lowConfidenceFields || [])),
      unmappedHeaders: unique(employeeTables.flatMap((item) => item.mapping.unmappedHeaders || [])),
      rows: employeeTables.flatMap((item) => item.mapping.rows || [])
    };
  }

  function detectTranslatedQuality(detectedTables, translated, mappingMeta) {
    const employeeRows = mappingMeta.rows || [];
    const quality = employeeRows.length ? detectQuality(employeeRows, translated.employees || [], mappingMeta) : { autoFixed: [], caution: [], risk: [], confidence: { score: 56, label: "低" } };

    if (!(translated.employees || []).length) {
      quality.risk.unshift(makeIssueItem("未识别到员工表", "-", "-", "employee_master", "缺少员工主表时，系统不能进入组织诊断主链路。"));
    }

    (detectedTables || []).forEach((table) => {
      if (table.key === "unknown") {
        quality.caution.push(makeIssueItem("有表未能自动识别类型", "-", "-", table.sheetName, `工作表 ${table.sheetName} 未进入标准分析模型。`));
        return;
      }
      if ((table.mapping.missingRequiredFields || []).length) {
        quality.caution.push(makeIssueItem("部分关键字段缺失", "-", "-", table.label, `${table.label}缺少 ${table.mapping.missingRequiredFields.join("、")}，会降低转译完整度。`));
      }
      if (table.confidence < 70) {
        quality.caution.push(makeIssueItem("表类型识别置信度偏低", "-", "-", table.label, `${table.sheetName} 通过相似命名识别为${table.label}，建议复核。`));
      }
    });

    (translated.recruitingReqs || []).forEach((row) => {
      if (row.internal_fill_flag !== "Y" && (row.time_to_fill_days <= 0 || row.time_to_fill_days < 10)) quality.caution.push(makeIssueItem("招聘周期值异常偏低", "-", row.req_id, "time_to_fill_days", "外部招聘岗位的招聘周期不应接近 0，请复核。"));
      if (row.total_cost <= 0 && row.internal_fill_flag !== "Y") quality.caution.push(makeIssueItem("招聘成本缺失或异常", "-", row.req_id, "total_cost", "外部招聘岗位建议补齐总招聘成本。"));
    });

    (translated.workforceMonthly || []).forEach((row) => {
      if (!row.month) quality.caution.push(makeIssueItem("月度快照缺少月份", "-", "-", "month", "无法纳入趋势分析。"));
      if (row.department_headcount <= 0) quality.caution.push(makeIssueItem("部门月末人数缺失", "-", row.department, "department_headcount", "会影响部门趋势和对比分析。"));
    });

    (translated.exitEvents || []).forEach((row) => {
      if (!row.employee_id) quality.caution.push(makeIssueItem("离职事件缺少员工编号", "-", row.exit_id, "employee_id", "会影响关键人才流失与替补分析。"));
    });

    let score = 95 - Math.min(quality.autoFixed.length, 14) - quality.caution.length * 2 - quality.risk.length * 5;
    if (!(translated.employees || []).length) score -= 20;
    score = clamp(score, 38, 97);
    quality.confidence = { score, label: score >= 82 ? "高" : score >= 68 ? "中" : "低" };
    return quality;
  }

  function buildIngestionMeta(sourceName, detectedTables, translated, quality) {
    const depth = depthFromData(translated);
    const translatedModels = [
      { key: "EmployeeCurrent", label: "员工现状主表", count: (translated.employees || []).length, detail: "支撑组织现状、人才盘点和基础继任分析" },
      { key: "WorkforceMonthly", label: "月度组织快照", count: (translated.workforceMonthly || []).length, detail: "支撑三年趋势、人数演化与覆盖变化" },
      { key: "RecruitingReq", label: "招聘需求表", count: (translated.recruitingReqs || []).length, detail: "支撑招聘周期、成本与效率分析" },
      { key: "ExitEvent", label: "离职事件表", count: (translated.exitEvents || []).length, detail: "支撑主动离职与关键人才流失分析" },
      { key: "CriticalRole", label: "关键岗位表", count: (translated.criticalRoles || []).length, detail: "支撑关键岗位暴露和覆盖深度分析" }
    ].filter((item) => item.count > 0);

    const impacts = [];
    if (!translated.recruitingReqs.length) impacts.push("未检测到招聘表，本轮不输出招聘周期、招聘成本与招聘效率。");
    if (!translated.workforceMonthly.length && !translated.exitEvents.length) impacts.push("未检测到月度快照或离职表，本轮不输出三年趋势与组织演化分析。");
    if (quality.risk.length) impacts.push("关键风险会优先影响继任判断、部门对比和趋势解释。");
    if (!impacts.length) impacts.push("当前上传数据已具备进入完整分析的最低质量要求。");

    const verdict = !depth.canAnalyze
      ? { label: "需要补充员工表后再分析", detail: "当前未识别到员工主表，系统已完成识别与转译，但暂不能进入分析结果。", tone: "danger" }
      : quality.confidence.score >= 82
      ? { label: "可以直接进入分析结果", detail: "识别与转译已完成，当前质量足以进入正式分析。", tone: "safe" }
      : quality.confidence.score >= 68
      ? { label: "可以分析，但要带风险提示", detail: "系统已完成转译，建议在汇报时对低置信字段和高风险项做提示。", tone: "warning" }
      : { label: "建议先看转译与质量影响", detail: "当前已经进入标准模型，但建议先看质量影响再讲结论。", tone: "warning" };

    return {
      sourceFiles: [sourceName],
      detectedTables: detectedTables.map((table) => ({
        ...
        matchedFields: (table.mapping.matchedFields || []).length,
        lowConfidenceFields: (table.mapping.lowConfidenceFields || []).length,
        missingRequiredFields: table.mapping.missingRequiredFields || []
      })),
      translatedModels,
      analysisDepth: depth,
      verdict,
      impacts,
      canAnalyze: depth.canAnalyze
    };
  }

  function translateUploadedDatasets(datasets, sourceName) {
    const detectedTables = (datasets || []).filter((item) => (item.rows || []).length).map((item) => {
      const detected = detectTableType(item.rows || [], item.name || "Sheet1");
      return { sheetName: item.name || "Sheet1", rowCount: item.rows.length, ...detected };
    });
    ...
    const employeeMappedRows = [];
    detectedTables.forEach((table) => {
      if (table.key === "employee_master") employeeMappedRows.push(...(table.mapping.rows || []));
      if (table.key === "workforce_monthly") translated.workforceMonthly.push(...translateWorkforceMonthly(table.mapping.rows || []));
      if (table.key === "recruiting_reqs") translated.recruitingReqs.push(...translateRecruitingReqs(table.mapping.rows || []));
      if (table.key === "exit_events") translated.exitEvents.push(...translateExitEvents(table.mapping.rows || []));
      if (table.key === "critical_roles") translated.criticalRoles.push(...translateCriticalRoles(table.mapping.rows || []));
    });
    translated.employees = enrichEmployees(employeeMappedRows);

    const mappingMeta = mergeEmployeeMapping(detectedTables);
    const quality = detectTranslatedQuality(detectedTables, translated, mappingMeta);
    const ingestion = buildIngestionMeta(sourceName, detectedTables, translated, quality);
    return { translated, mappingMeta, quality, ingestion, detectedTables };
  }
  ...
  function buildIssues(rows, summary) {
    ...
    if (analysisEngine && analysisEngine.buildDiagnostics) {
      return analysisEngine.buildDiagnostics({ rows, departmentSummary: summary, roles, criticalRoles: state.criticalRoles, workforceMonthly: state.workforceMonthly, recruitingReqs: state.recruitingReqs, exitEvents: state.exitEvents }).issues.map((item) => ({
        title: item.title,
  ...
  function executiveSummary(rows) {
    ...
    const roles = getCriticalRoles(rows);
    const diagnostics = analysisEngine && analysisEngine.buildDiagnostics ? analysisEngine.buildDiagnostics({ rows, departmentSummary: summary, roles, criticalRoles: state.criticalRoles, workforceMonthly: state.workforceMonthly, recruitingReqs: state.recruitingReqs, exitEvents: state.exitEvents }) : null;
  ...
  function recruitingMonthlySeries() {
    ...
    return months.map((month) => {
      ...
      return {
        ...
        offerAcceptRate: Number((rows.reduce((sum, item) => sum + Number(item.offer_acceptance_rate || 0), 0) / Math.max(1, rows.length)).toFixed(1)),
  ...
  function operationsDepartmentCards() {
    ...
    return overviewDepartmentsV5(executiveSummary(state.employees)).map((item) => {
      ...
      const reqRows = (state.recruitingReqs || []).filter((req) => req.department === item.name && String(req.open_date || "").startsWith("2025"));
      const ttf = average(reqRows, (req) => Number(req.time_to_fill_days || 0));
      const avgCost = average(reqRows, (req) => Number(req.total_cost || 0));
      const offerAccept = average(reqRows, (req) => Number(req.offer_acceptance_rate || 0));
      return {
        name: item.name,
        ...
        avgCost: Math.round(avgCost || 0),
        offerAccept: Number((offerAccept || 0).toFixed(1)),
  ...
  function renderHomeV7() {
    return `<div class="home-entry-v8"><header class="home-entry-header-v8"><a class="home-brand-v8" href="#home" aria-label="TalentPulse 首页"><span class="home-brand-mark-v8">TP</span><span class="home-brand-text-v8">TalentPulse</span></a><nav class="home-nav-v8"><a href="#overview">官方案例</a><a href="#operations">组织运营</a><a href="#report">汇报报告</a></nav></header><main class="home-entry-main-v8"><div class="home-entry-copy-v8"><h1>组织诊断工作台</h1><p>上传员工数据，或直接进入官方案例。</p><div class="home-entry-actions-v8"><button class="btn btn-primary" data-action="try-demo">进入 Demo</button><label class="btn btn-secondary upload-inline">上传数据<input id="upload-input" type="file" accept=".csv,.xlsx,.xls" hidden></label></div><small>支持 Excel / CSV 自动识别常见人力字段</small></div></main></div>`;
  }
  ...
  function renderOverviewV7() {
    ...
    const linkedDepartments = overviewIssueDepartmentsV5(summary, selectedIssue);
    const linkedCards = departments.filter((item) => linkedDepartments.includes(item.name));
    const currentIssue = issues.find((item) => (item.departments || []).includes(currentDepartment ? currentDepartment.name : "")) || selectedIssue;
    const evidenceLinks = visibleRoutes().filter((route) => ["operations", "review", "succession", "health", "report"].includes(route.key)).map((route) => ({ route: route.key, label: "查看更多证据", title: route.label }));
    const deptMetrics = currentDepartment ? [
      { label: "人才密度", value: currentDepartment.talentDensity, color: "var(--v8-blue)" },
      { label: "高潜比例", value: Math.round((currentDepartment.item.hipoA / Math.max(1, currentDepartment.item.count)) * 100), color: "#4b82ff" },
      { label: "近端接班", value: Math.round((currentDepartment.item.readyNow / Math.max(1, currentDepartment.item.count)) * 100), color: "#7aa2ff" },
      { label: "关键岗位覆盖", value: currentDepartment.coverage, color: "var(--v8-ink)" }
    ] : [];
    return `<section class="hero-stage-v8 overview-stage-top-v8"><div class="hero-main-v8"><div class="brand-kicker-v8">公司现状</div><h2>${safe(summary.headline)}</h2><p>${safe(summary.state)}</p><div class="hero-meta-v8"><span class="hero-risk-v8 ${summary.riskLevel === "高" ? "danger" : summary.riskLevel === "中高" ? "warning" : "safe"}">风险等级：${safe(summary.riskLevel)}</span><div class="hero-priority-box-v8"><span>当前优先动作</span><strong>${safe(summary.priority)}</strong></div></div></div><div class="hero-side-v8">${issues.map((item) => `<div class="hero-side-row-v8"><span>${safe(item.title)}</span><strong>${safe(item.managementRead)}</strong></div>`).join("")}</div></section>${renderKpiStripV8(summary, annual)}<section class="stage-section-v8"><div class="stage-title-v8"><span>部门现状</span><h3>先看部门，再收束问题</h3></div><div class="department-stage-v8"><div class="department-list-v8">${departments.map((item) => `<button class="department-pill-v8 ${currentDepartment && currentDepartment.name === item.name ? "active" : ""}" data-overview-department="${safe(item.name)}"><span>${safe(item.name)}</span><strong>${safe(item.judgment)}</strong><small>${safe(`人数 ${item.item.count} · 近端接班 ${item.item.readyNow} · 覆盖 ${item.coverage}%`)}</small></button>`).join("")}</div><div class="department-canvas-v8"><div class="canvas-top-v8"><div><span>${safe(currentDepartment ? currentDepartment.name : "")}</span><h3>${safe(currentDepartment ? currentDepartment.judgment : "暂无部门样本")}</h3></div><p>${safe(currentIssue ? `当前更值得关注的问题是：${currentIssue.title}` : "当前没有可展示的问题样本。")}</p></div><div class="canvas-chart-v8">${renderDepartmentMatrixV7(departments)}</div><div class="canvas-bottom-v8"><div class="canvas-bars-v8">${currentDepartment ? renderBars(deptMetrics) : `<div class="empty-state compact-empty">暂无部门数据。</div>`}</div><div class="canvas-note-v8"><strong>${safe(currentDepartment ? currentDepartment.name : "部门")}</strong><p>${safe(currentDepartment ? `${currentDepartment.name}当前的人才密度、高潜比例、近端接班和岗位覆盖没有同时向好，因此更适合从结构而不是单一员工去看问题。` : "请选择部门查看右侧证据。")}</p></div></div></div></div></section><section class="stage-section-v8"><div class="stage-title-v8"><span>关键问题</span><h3>只保留 3 个最值得讲的问题</h3></div><div class="issue-selectors-v8">${issues.map((item, index) => `<button class="issue-switch-v8 ${selectedIssue && selectedIssue.title === item.title ? "active" : ""}" data-overview-issue="${index}" data-overview-issue-dept="${safe((item.departments && item.departments[0]) || "")}"><span>${safe(`问题 ${index + 1}`)}</span><strong>${safe(item.title)}</strong><small>${safe(item.managementRead)}</small></button>`).join("")}</div><div class="issue-canvas-v8"><div class="issue-evidence-v8"><span>问题由哪些部门现状构成</span><h3>${safe(selectedIssue ? selectedIssue.title : "")}</h3><p>${safe(selectedIssue ? selectedIssue.phenomenon : "")}</p>${renderProblemEvidenceV7(selectedIssue || {}, linkedCards)}</div><div class="issue-cause-v8"><span>根因解释</span><strong>${safe(selectedIssue ? selectedIssue.rootCause : "")}</strong><div class="issue-metrics-v8">${linkedCards.length ? renderBars(issueEvidenceMetricsV6(selectedIssue || {}, linkedCards).map((item) => ({ label: item.label, value: item.value, color: item.tone === "danger" ? "#ef4444" : item.tone === "warning" ? "#f59e0b" : "var(--v8-blue)" }))) : `<div class="empty-state compact-empty">当前暂无可关联的指标异常。</div>`}</div></div></div></section><section class="stage-section-v8"><div class="stage-title-v8"><span>风险与建议</span><h3>风险和动作绑定问题联动</h3></div>${renderIssueLinkageV8(selectedIssue)}</section>${renderEvidenceLinksV6(evidenceLinks)}`;
  }
  ...
  function renderOperationsV7() {
    ...
    const chartCards = [];
    if (company.length) {
      chartCards.push({ title: "月末人数", note: "2023-2025", values: company.map((item) => item.companyHeadcount), label: "公司人数" });
      chartCards.push({ title: "自愿离职率", note: "月度均值", values: company.map((item) => item.voluntaryAttritionRate), label: "主动离职率" });
      chartCards.push({ title: "关键人才流失率", note: "关键损失", values: company.map((item) => item.regrettableAttritionRate), label: "关键人才流失率" });
    }
    if (recruiting.length) {
      chartCards.push({ title: "平均招聘周期", note: "按月需求均值", values: recruiting.map((item) => item.avgTimeToFill), label: "招聘周期" });
      chartCards.push({ title: "平均招聘成本", note: "单需求均值", values: recruiting.map((item) => item.avgCost), label: "招聘成本" });
      chartCards.push({ title: "内部补位率", note: "关键岗位内部供给", values: recruiting.map((item) => item.internalFillRate), label: "内部补位率" });
      chartCards.push({ title: "Offer 接受率", note: "按月均值", values: recruiting.map((item) => item.offerAcceptRate), label: "Offer 接受率" });
    }
    const departmentBars = currentDepartment ? (state.workforceMonthly.length ? [
      { label: "招聘周期", value: currentDepartment.ttf || 0, color: "var(--v8-blue)" },
      { label: "主动离职率", value: currentDepartment.attrition || 0, color: "#ef4444" },
      { label: "近端接班覆盖", value: currentDepartment.readiness || 0, color: "#7aa2ff" },
      { label: "月末人数", value: currentDepartment.headcount || 0, color: "var(--v8-ink)" }
    ] : [
      { label: "招聘周期", value: currentDepartment.ttf || 0, color: "var(--v8-blue)" },
      { label: "平均招聘成本", value: currentDepartment.avgCost || 0, color: "var(--v8-ink)" },
      { label: "Offer 接受率", value: currentDepartment.offerAccept || 0, color: "#7aa2ff" }
    ]) : [];
    return `<section class="hero-stage-v8 compact-stage-v8"><div class="hero-main-v8"><div class="brand-kicker-v8">组织运营</div><h2>${safe(state.workforceMonthly.length ? "过去三年的招聘与人员流动，解释今天的组织状态" : "当前已接入招聘数据，可先看招聘周期、成本与供给效率")}</h2><p>${safe(state.workforceMonthly.length ? "时间范围覆盖 2023-01 至 2025-12，所有趋势都基于真实月度模拟数据。" : "当前未检测到月度快照或离职表，因此本页只展开招聘侧证据。")}</p></div></section><section class="ops-grid-v8">${chartCards.map((item) => `<article class="chart-stage-v7"><div class="section-head"><h3>${safe(item.title)}</h3><span>${safe(item.note)}</span></div>${renderLineChartV7(item.values, item.label)}</article>`).join("") || `<div class="empty-state">当前没有可展示的组织运营数据。</div>`}</section><section class="stage-section-v8"><div class="stage-title-v8"><span>部门运营证据</span><h3>点击部门，看当前阶段最有价值的运营信号</h3></div><div class="department-stage-v8"><div class="department-list-v8">${cards.map((item) => `<button class="department-pill-v8 ${currentDepartment && currentDepartment.name === item.name ? "active" : ""}" data-operations-department="${safe(item.name)}"><span>${safe(item.name)}</span><strong>${safe(`招聘周期 ${item.ttf || 0} 天`)}</strong><small>${safe(state.workforceMonthly.length ? `人数 ${item.headcount} · 离职率 ${item.attrition}% · 近端覆盖 ${item.readiness}%` : `平均成本 ${item.avgCost} · 接受率 ${item.offerAccept}%`)}</small></button>`).join("")}</div><div class="department-canvas-v8"><div class="canvas-top-v8"><div><span>${safe(currentDepartment ? currentDepartment.name : "")}</span><h3>当前阶段运营证据</h3></div><p>${safe(state.workforceMonthly.length ? (currentDepartment && currentDepartment.attrition >= 4 ? "离职率和招聘周期都偏高，外部补员压力与内部供给不足同时存在。" : currentDepartment && currentDepartment.readiness <= 10 ? "招聘效率已回落，但近端覆盖仍然偏薄，问题不只在招不到人，也在内部供给。" : "当前运营指标相对平稳，更需要关注结构性供给而不是单月波动。") : "当前展示的是招聘侧证据，重点观察招聘周期、成本和接受率的组合。")}</p></div><div class="canvas-bars-v8">${currentDepartment ? renderBars(departmentBars) : `<div class="empty-state compact-empty">暂无部门运营数据。</div>`}</div></div></div></section>`;
  }
  ...
  function renderTalentReviewV7() {
    ...
    const cellEmployees = (selectedCell ? selectedCell.employees : []).slice().sort((a, b) => b.shl.score - a.shl.score);
    const selectedEmployee = findEmployee(state.selectedEmployeeId) || cellEmployees[0] || rows[0] || null;
    const densityItems = departments.map((item) => ({ label: item.name.replace("中心", ""), value: item.talentDensity, color: "var(--v8-blue)" }));
    ...
    const gapItems = departments.map((item) => ({ label: item.name.replace("中心", ""), value: Math.max(0, item.talentDensity - Math.round(((item.item.readyNow || 0) / Math.max(1, item.item.count || 1)) * 100)), color: item.talentDensity >= 10 && item.item.readyNow <= 4 ? "#ef4444" : "var(--v8-ink)" }));
    return `<section class="hero-stage-v8 compact-stage-v8"><div class="hero-main-v8"><div class="brand-kicker-v8">人才盘点</div><h2>${safe(summary.kpis.hipoA > summary.kpis.readyNow ? "人才储备并不差，但梯队成熟度弱于绩效表象" : "人才密度总体可用，但不同团队之间的梯队转化并不均衡")}</h2><p>${safe(`当前样本 ${rows.length} 人，高潜 A ${summary.kpis.hipoA}，现在可接任 ${summary.kpis.readyNow}。`)}</p></div></section><section class="tri-visual-v8"><article class="chart-stage-v7"><div class="section-head"><h3>人才密度排名</h3><span>看密度，而不是只看总人数</span></div>${renderBars(densityItems)}</article><article class="chart-stage-v7"><div class="section-head"><h3>高绩效 / 高潜错位</h3><span>看起来强，并不等于后备成熟</span></div>${renderScatterV6(mismatchPoints)}</article><article class="chart-stage-v7"><div class="section-head"><h3>梯队断层排序</h3><span>人才密度减去近端接班</span></div>${renderBars(gapItems)}</article></section><section class="stage-section-v8"><div class="stage-title-v8"><span>九宫格证据</span><h3>九宫格只作为辅助证据</h3></div><div class="filter-row"><select class="filter-select" data-filter="department"><option value="All">全部部门</option>${unique(state.employees.map((row) => row.department)).map((department) => `<option value="${safe(department)}" ${state.selectedDepartment === department ? "selected" : ""}>${safe(department)}</option>`).join("")}</select><select class="filter-select" data-filter="jobLevel"><option value="All">全部职级</option>${unique(state.employees.map((row) => row.job_level)).map((level) => `<option value="${safe(level)}" ${state.selectedJobLevel === level ? "selected" : ""}>${safe(level)}</option>`).join("")}</select></div><div class="ninebox-grid push-top">${cells.map((cell) => `<button class="ninebox-cell ${selectedCell && selectedCell.key === cell.key ? "active" : ""}" data-ninebox="${safe(cell.key)}"><span class="cell-title">${safe(cell.title)}</span><strong>${safe(cell.employees.length)}</strong><small>${safe(cell.key === "3-3" ? "高潜集中区" : cell.key === "3-2" ? "高绩效但梯队未成熟" : "结构证据")}</small></button>`).join("")}</div></section>${renderEmployeeDrawerV5("查看代表样本", cellEmployees, selectedEmployee)}`;
  }
  ...
  function renderSuccessionV7() {
    ...
    const selectedEmployee = findEmployee(state.selectedEmployeeId) || (selectedRole && selectedRole.candidates[0]) || null;
    const singlePoints = singlePointRoles(roles);
    const uncoveredRank = roles.filter((role) => role.incumbent.successor_nomination_flag !== "Y" || !role.candidates.length).slice(0, 8).map((role) => ({ label: role.position, value: Math.round(role.riskScore), color: "#ef4444" }));
    const weakCoverageRank = roles.filter((role) => role.candidates.length && (role.candidates.length < 2 || role.candidates[0].succession.band !== "Ready Now")).slice(0, 8).map((role) => ({ label: role.position, value: role.candidates.length, color: "#4b82ff" }));
    const dependencyRank = singlePoints.slice(0, 8).map((role) => ({ label: role.position, value: Math.round(role.riskScore), color: "var(--v8-ink)" }));
    const supportRoles = supportRiskRoles(roles);
    return `<section class="hero-stage-v8 compact-stage-v8"><div class="hero-main-v8"><div class="brand-kicker-v8">继任分析</div><h2>${safe(summary.kpis.coverage < 70 ? "关键岗位覆盖已经可见，但后备深度仍然偏薄" : "名义覆盖可用，但过多岗位仍然依赖单一候选人")}</h2><p>${safe(`关键岗位 ${summary.kpis.criticalRoles} 个，已覆盖 ${summary.kpis.coveredCriticalRoles} 个，无后备 ${summary.kpis.uncovered} 个。`)}</p></div></section><section class="tri-visual-v8"><article class="chart-stage-v7"><div class="section-head"><h3>无后备岗位</h3><span>先看最直接的空窗</span></div>${renderBars(uncoveredRank.length ? uncoveredRank : [{ label: "当前无明显空窗", value: 1, color: "var(--v8-blue)" }])}</article><article class="chart-stage-v7"><div class="section-head"><h3>名义覆盖但弱</h3><span>覆盖数量掩盖覆盖深度</span></div>${renderBars(weakCoverageRank.length ? weakCoverageRank : [{ label: "当前弱覆盖较少", value: 1, color: "#4b82ff" }])}</article><article class="chart-stage-v7"><div class="section-head"><h3>单点依赖排行</h3><span>真正危险的是只靠一个人</span></div>${renderBars(dependencyRank.length ? dependencyRank : [{ label: "当前单点依赖较少", value: 1, color: "var(--v8-ink)" }])}</article></section><section class="stage-section-v8"><div class="stage-title-v8"><span>支持职能暴露</span><h3>人数不多，但断档成本更高</h3></div><div class="support-grid-v8">${supportRoles.length ? supportRoles.map((role) => `<div class="support-card-v8"><span>${safe(role.department)}</span><strong>${safe(role.position)}</strong><p>${safe(role.candidates[0] ? `${role.candidates[0].name} 是当前最佳候选，准备度 ${readinessText(role.candidates[0].succession.band)}。` : "当前没有明确候选，需要优先命名后备。")}</p></div>`).join("") : `<div class="empty-state">当前没有可展示的支持职能关键岗位。</div>`}</div></section><section class="stage-section-v8 secondary-stage-v8"><div class="stage-title-v8"><span>辅助证据</span><h3>岗位暴露热区</h3></div><div class="heatmap-grid">${roles.slice(0, 9).map((role) => { const best = role.candidates[0]; const alpha = Math.max(0.2, Math.min(0.88, (best ? best.succession.score : 36) / 100)); return `<button class="heat-cell ${selectedRole && selectedRole.key === role.key ? "active" : ""}" data-role="${safe(role.key)}" style="background: rgba(47,107,255,${alpha})"><strong>${safe(role.department)}</strong><span>${safe(role.position)}</span><em>${safe(best ? readinessText(best.succession.band) : "暂无后备")}</em></button>`; }).join("")}</div></section>${renderProfileDrawer(selectedEmployee, "查看岗位与候选人样本")}`;
  }
  ...
  function renderHealthV7() {
    ...
    const verdict = ingestion.verdict && ingestion.verdict.label ? ingestion.verdict.label : (quality.confidence.score >= 82 ? "可以直接演示" : quality.confidence.score >= 68 ? "可以分析，但要带风险提示" : "需要先清理再汇报");
    const depth = ingestion.analysisDepth || currentAnalysisDepth();
    const understoodRate = ingestion.detectedTables && ingestion.detectedTables.length ? Math.round(average(ingestion.detectedTables, (item) => Number(item.confidence || 0))) : Math.round((state.mappingMeta.matchedFields.length / Math.max(1, FIELDS.length)) * 100);
    const totalIssues = quality.autoFixed.length + quality.caution.length + quality.risk.length;
    const afterIssues = quality.caution.length + quality.risk.length;
    const detectedTables = (ingestion.detectedTables || []).map((item) => `<div class="translation-item-v8"><strong>${safe(item.label)}</strong><span>${safe(`${item.sheetName} · ${item.rows} 行 · 置信度 ${item.confidence}%`)}</span></div>`).join("") || `<div class="translation-item-v8"><strong>未检测到上传表</strong><span>当前展示的是内置样本。</span></div>`;
    const translatedModels = (ingestion.translatedModels || []).map((item) => `<div class="translation-item-v8"><strong>${safe(item.label)}</strong><span>${safe(`${item.key} · ${item.count} 条`)}</span></div>`).join("") || `<div class="translation-item-v8"><strong>暂无标准模型</strong><span>请先上传员工表。</span></div>`;
    const cautionText = state.mappingMeta.lowConfidenceFields.length ? state.mappingMeta.lowConfidenceFields.slice(0, 3).join("、") : "当前低置信字段较少";
    const impactText = (ingestion.impacts || []).slice(0, 3).map((item) => `<div class="impact-chip-v8">${safe(item)}</div>`).join("");
    return `<section class="hero-stage-v8 compact-stage-v8"><div class="hero-main-v8"><div class="brand-kicker-v8">识别与转译</div><h2>${safe(verdict)}</h2><p>${safe(ingestion.verdict && ingestion.verdict.detail ? ingestion.verdict.detail : `已识别 ${state.mappingMeta.matchedFields.length} 个分析字段，关键风险 ${quality.risk.length} 项。`)}</p></div></section><section class="translation-flow-v8"><div class="translation-step-v8 done"><span>1</span><strong>上传</strong><small>${safe((ingestion.sourceFiles && ingestion.sourceFiles[0]) || state.sourceName)}</small></div><div class="translation-step-v8 done"><span>2</span><strong>识别并转译</strong><small>${safe(`${(ingestion.detectedTables || []).length || 1} 张表进入标准模型`)}</small></div><div class="translation-step-v8 ${depth.canAnalyze ? "active" : ""}"><span>3</span><strong>分析结果</strong><small>${safe(depth.label)}</small></div>${depth.canAnalyze ? `<button class="btn btn-primary" data-action="open-analysis">进入分析结果</button>` : `<span class="status warning">请先补充员工表</span>`}</section><section class="health-stage-v8"><article class="health-panel-v8 health-panel-main-v8"><span>可信度结论</span><strong>${safe(`${quality.confidence.score} / 100`)}</strong><p>${safe(verdict)}</p>${renderMeterV8(quality.confidence.score, "数据可用等级", quality.confidence.score >= 82 ? "safe" : quality.confidence.score >= 68 ? "warning" : "danger")}</article><article class="health-panel-v8"><span>自动识别了什么</span><strong>${safe(`${(ingestion.detectedTables || []).length} 张表`)}</strong>${renderMeterV8(understoodRate, "识别置信度", "info")}<div class="translation-stack-v8">${detectedTables}</div></article><article class="health-panel-v8"><span>已转译什么</span><strong>${safe(`${(ingestion.translatedModels || []).length} 个标准模型`)}</strong><p>${safe(depth.detail)}</p><div class="translation-stack-v8">${translatedModels}</div></article><article class="health-panel-v8"><span>质量影响</span><strong>${safe(`${quality.caution.length + quality.risk.length} 项`)}</strong>${renderStackBarsV8([{ label: "已自动修复", value: quality.autoFixed.length, tone: "safe" }, { label: "需谨慎", value: quality.caution.length, tone: "warning" }, { label: "关键风险", value: quality.risk.length, tone: "danger" }])}</article></section><section class="health-copy-grid-v8"><article class="text-panel-v8"><span>自动修复了什么</span><strong>${safe("字段归一、日期修正与重复清洗已自动完成")}</strong><p>${safe(normalizationActions().join("；"))}</p></article><article class="text-panel-v8"><span>哪些地方需要谨慎</span><strong>${safe(cautionText)}</strong><p>${safe(quality.risk.length ? "这些问题会优先影响继任判断、部门对比和趋势解释。" : "当前结果足以直接进入组织诊断与汇报。")}</p></article><article class="text-panel-v8"><span>这些问题会影响什么</span><strong>${safe(depth.canAnalyze ? depth.label : "待补充")}</strong><div class="impact-list-v8">${impactText || `<div class="impact-chip-v8">当前没有额外影响提示。</div>`}</div></article></section><section class="appendix-table-v8"><div class="section-head"><h3>附录证据</h3><span>问题样本下沉，不抢主视觉</span></div><div class="tabs"><button class="tab ${state.activeHealthTab === "autoFixed" ? "active" : ""}" data-health-tab="autoFixed">已自动修复</button><button class="tab ${state.activeHealthTab === "caution" ? "active" : ""}" data-health-tab="caution">需谨慎解读</button><button class="tab ${state.activeHealthTab === "risk" ? "active" : ""}" data-health-tab="risk">关键风险</button></div><table class="data-table compact"><thead><tr><th>问题</th><th>字段</th><th>对象</th></tr></thead><tbody>${rows.length ? rows.slice(0, 6).map((item, index) => `<tr class="${state.selectedHealthIndex === index ? "table-row-active" : ""}" data-health-index="${index}"><td>${safe(item.title)}</td><td>${safe(item.field)}</td><td>${safe(item.employeeId)}</td></tr>`).join("") : `<tr><td colspan="3"><div class="empty-state compact-empty">当前分类暂无问题。</div></td></tr>`}</tbody></table></section>`;
  }
  ...
  function renderReportV7() {
    ...
    const issues = topIssueCards(summary).slice(0, 3);
    const annual = annualCompareSeries();
    return `<section class="hero-stage-v8 report-hero-v8"><div class="hero-main-v8"><div class="brand-kicker-v8">汇报报告</div><h2>${safe(summary.headline)}</h2><p>${safe(summary.state)}</p></div><div class="hero-side-v8"><div class="hero-side-row-v8"><span>风险等级</span><strong>${safe(summary.riskLevel)}</strong></div><div class="hero-side-row-v8"><span>当前优先动作</span><strong>${safe(summary.priority)}</strong></div></div></section><section class="executive-strip-v8"><div><span>公司现状</span><strong>${safe(summary.state)}</strong></div><div><span>部门现状摘要</span><strong>${safe("差异主要集中在研发中心、销售增长、交付运营、产品与设计，以及低可见度支持职能。")}</strong></div><div><span>当前优先动作</span><strong>${safe(summary.priority)}</strong></div></section><section class="issue-board-v8">${issues.map((item) => `<article class="issue-board-card-v8"><span>${safe(item.title)}</span><div><strong>现状表现</strong><p>${safe(item.phenomenon)}</p></div><div><strong>根因解释</strong><p>${safe(item.rootCause)}</p></div><div><strong>风险外推</strong><p>${safe(item.likelyConsequence)}</p></div><div><strong>建议动作</strong><p>${safe(item.judgment)}</p></div></article>`).join("")}</section><section class="stage-section-v8"><div class="stage-title-v8"><span>风险—建议映射</span><h3>把风险和动作收在一张图里</h3></div><div class="issue-linkage-v8">${issues.map((item) => `<div class="issue-linkage-step-v8"><span>${safe(item.title)}</span><strong>${safe(item.likelyConsequence)}</strong><p>${safe(item.actionNow || item.judgment)}</p></div>`).join("")}</div></section>${renderEvidenceFootnotesV8(annual)}`;
  }

  function renderMiniColumnsV8(items, formatter) {
    const values = (items || []).map((item) => Number(item || 0));
    const max = Math.max(1, ...values);
    return `<div class="mini-columns-v8">${values.map((item, index) => `<div class="mini-column-v8"><div class="mini-column-bar-v8"><div style="height:${Math.max(12, (item / max) * 100)}%"></div></div><span>${safe(["2023", "2024", "2025"][index] || "")}</span><strong>${safe(formatter ? formatter(item) : item)}</strong></div>`).join("")}</div>`;
  }

  function renderKpiStripV8(summary, annual) {
    const current = annual[annual.length - 1] || { headcount: state.employees.length, attrition: 0, readyNowCoverage: summary.kpis.coverage, criticalGap: summary.kpis.uncovered };
    const items = [
      { label: "月末人数", value: `${current.headcount} 人`, note: "真实三年轨迹", values: annual.map((item) => item.headcount), formatter: (value) => `${value}` },
      { label: "主动离职率", value: `${current.attrition}%`, note: "2024 抬升，2025 回落", values: annual.map((item) => item.attrition), formatter: (value) => `${value}%` },
      { label: "近端接班覆盖", value: `${current.readyNowCoverage}%`, note: "覆盖率没有跟着扩张同步抬升", values: annual.map((item) => item.readyNowCoverage), formatter: (value) => `${value}%` },
      { label: "关键岗位空窗", value: `${current.criticalGap}`, note: "暴露更多来自结构", values: annual.map((item) => item.criticalGap), formatter: (value) => `${value}` }
    ];
    return `<section class="kpi-strip-v8">${items.map((item) => `<article class="kpi-stage-v8"><span>${safe(item.label)}</span><strong>${safe(item.value)}</strong><small>${safe(item.note)}</small>${renderMiniColumnsV8(item.values, item.formatter)}</article>`).join("")}</section>`;
  }

  function renderMeterV8(value, label, toneClass) {
    const score = Math.max(0, Math.min(100, Number(value || 0)));
    return `<div class="meter-v8 ${safe(toneClass || "")}"><div class="meter-head-v8"><span>${safe(label || "")}</span><strong>${safe(`${score}%`)}</strong></div><div class="meter-track-v8"><div class="meter-fill-v8" style="width:${score}%"></div></div></div>`;
  }

  function renderStackBarsV8(items) {
    const total = Math.max(1, (items || []).reduce((sum, item) => sum + Number(item.value || 0), 0));
    return `<div class="stack-bars-v8">${(items || []).map((item) => `<div class="stack-row-v8"><span>${safe(item.label)}</span><div class="stack-track-v8"><div class="stack-fill-v8 ${safe(item.tone || "")}" style="width:${(Number(item.value || 0) / total) * 100}%"></div></div><strong>${safe(item.value)}</strong></div>`).join("")}</div>`;
  }

  function renderIssueLinkageV8(issue) {
    if (!issue) return `<div class="empty-state">当前暂无问题样本。</div>`;
    const steps = [
      { label: "风险外推", text: issue.likelyConsequence || issue.predict || "当前问题若不处理，会沿着关键岗位和核心团队继续放大。" },
      { label: "立即动作", text: issue.actionNow || issue.judgment || "先核验关键岗位和高风险核心人群。" },
      { label: "中期动作", text: issue.actionSoon || issue.judgment || "围绕关键团队补齐近中期候选。" },
      { label: "梯队建设", text: issue.actionLater || issue.judgment || "把风险治理沉淀成持续供给机制。" }
    ];
    return `<div class="issue-linkage-v8">${steps.map((step) => `<div class="issue-linkage-step-v8"><span>${safe(step.label)}</span><strong>${safe(step.text)}</strong></div>`).join("")}</div>`;
  }

  function renderEvidenceFootnotesV8(annual) {
    const items = [
      { title: "人数变化", key: "headcount", unit: "人" },
      { title: "主动离职率", key: "attrition", unit: "%" },
      { title: "关键岗位空窗", key: "criticalGap", unit: "" }
    ];
    return `<section class="footnote-evidence-v8">${items.map((item) => `<article class="footnote-card-v8"><span>${safe(item.title)}</span>${renderMiniColumnsV8(annual.map((year) => year[item.key]), (value) => `${value}${item.unit}`)}</article>`).join("")}</section>`;
  }
  ...
  function renderShell() {
    ...
    if (homeMode) {
      return `<div class="home-stage-v8">${content}</div>`;
    }
    const showActions = state.route !== "report";
    return `<div class="shell shell-v8"><aside class="sidebar sidebar-v8"><div class="brand"><div class="brand-mark">TP</div><div><div class="brand-title">TalentPulse</div><div class="brand-sub">中文组织诊断台</div></div></div><nav class="nav">${visibleRoutes().map((route) => `<a class="nav-item ${state.route === route.key ? "active" : ""}" href="#${route.key}">${safe(route.label)}</a>`).join("")}</nav><div class="sidebar-foot"><div class="small-label">当前数据</div><div class="sidebar-company">${safe(state.sourceName)}</div></div></aside><section class="main main-v8"><header class="topbar topbar-v8"><div><div class="page-eyebrow">公司现状 → 部门现状 → 关键问题 → 风险外推 → 建议动作</div><h1>${safe(routeLabel(state.route))}</h1></div><div class="topbar-actions">${showActions ? `<button class="btn btn-secondary" data-action="try-demo">官方案例</button><label class="btn btn-primary upload-inline">上传员工数据<input id="upload-input-top" type="file" accept=".csv,.xlsx,.xls" hidden></label>` : `<span class="status info">${safe(state.sourceName)}</span>`}</div></header><main class="content content-v8">${content}</main></section></div>`;
  }
  ...
  function useRows(rawRows, sourceName, mode, extras) {
    ...
    state.criticalRoles = (extras && extras.criticalRoles ? extras.criticalRoles.slice() : []);
  }

  function resetAnalysisSelections() {
  ...
  function refreshIngestionFromCurrent(sourceName) {
    const translated = {
      employees: state.employees.slice(),
      workforceMonthly: state.workforceMonthly.slice(),
      recruitingReqs: state.recruitingReqs.slice(),
      exitEvents: state.exitEvents.slice(),
      criticalRoles: state.criticalRoles.slice()
    };
    const detectedTables = [
      { sheetName: "员工主表", key: "employee_master", label: "员工表", model: "EmployeeCurrent", rowCount: translated.employees.length, confidence: 98, mapping: { matchedFields: FIELDS, lowConfidenceFields: [], missingRequiredFields: [] } },
      translated.workforceMonthly.length ? { sheetName: "月度组织快照", key: "workforce_monthly", label: "月度快照", model: "WorkforceMonthly", rowCount: translated.workforceMonthly.length, confidence: 96, mapping: { matchedFields: WORKFORCE_FIELDS, lowConfidenceFields: [], missingRequiredFields: [] } } : null,
      translated.recruitingReqs.length ? { sheetName: "招聘需求表", key: "recruiting_reqs", label: "招聘表", model: "RecruitingReq", rowCount: translated.recruitingReqs.length, confidence: 96, mapping: { matchedFields: RECRUITING_FIELDS, lowConfidenceFields: [], missingRequiredFields: [] } } : null,
      translated.exitEvents.length ? { sheetName: "离职事件表", key: "exit_events", label: "离职表", model: "ExitEvent", rowCount: translated.exitEvents.length, confidence: 96, mapping: { matchedFields: EXIT_FIELDS, lowConfidenceFields: [], missingRequiredFields: [] } } : null,
      translated.criticalRoles.length ? { sheetName: "关键岗位表", key: "critical_roles", label: "关键岗位表", model: "CriticalRole", rowCount: translated.criticalRoles.length, confidence: 96, mapping: { matchedFields: ROLE_FIELDS, lowConfidenceFields: [], missingRequiredFields: [] } } : null
    ].filter(Boolean);
    state.ingestion = buildIngestionMeta(sourceName, detectedTables, translated, state.quality);
  }

  function applyTranslatedUpload(payload, sourceName) {
    state.sourceName = sourceName;
    state.uploadMode = "upload";
    state.rawRows = payload.mappingMeta.rows.slice();
    state.mappingMeta = payload.mappingMeta;
    state.employees = payload.translated.employees.slice();
    state.workforceMonthly = payload.translated.workforceMonthly.slice();
    state.recruitingReqs = payload.translated.recruitingReqs.slice();
    state.exitEvents = payload.translated.exitEvents.slice();
    state.criticalRoles = payload.translated.criticalRoles.slice();
    state.quality = payload.quality;
    state.ingestion = payload.ingestion;
    resetAnalysisSelections();
    state.activeHealthTab = state.quality.risk.length ? "risk" : state.quality.caution.length ? "caution" : "autoFixed";
    state.uploadNote = payload.ingestion.verdict.detail;
    state.route = "health";
  ...
  function handleDemo() {
    useRows((demo.clean && (demo.clean.employee_master || demo.clean.employees)) || [], "官方讲述版", "demo", {
      ...
      criticalRoles: (demo.clean && demo.clean.critical_roles) || []
    });
    resetAnalysisSelections();
    refreshIngestionFromCurrent("官方讲述版");
  ...
  function handleDirtyDemo() {
    useRows((demo.dirty && (demo.dirty.employee_master || demo.dirty.employees)) || [], "AI 清洗演示版", "upload", {
      ...
      criticalRoles: (demo.dirty && demo.dirty.critical_roles) || (demo.clean && demo.clean.critical_roles) || []
    });
    resetAnalysisSelections();
    refreshIngestionFromCurrent("AI 清洗演示版");
  ...
  function useUploadedDatasets(datasets, filename) {
    const nonEmpty = (datasets || []).filter((item) => (item.rows || []).length);
    if (!nonEmpty.length) {
    ...
    applyTranslatedUpload(translateUploadedDatasets(nonEmpty, filename), filename);
  }

  function handleFile(file) {
    if (!file) return;
    if (/\.csv$/i.test(file.name)) {
      file.text().then((text) => useUploadedDatasets([{ name: file.name, rows: parseCsv(text) }], file.name));
      return;
    }
    if (/\.xlsx?$/i.test(file.name) && window.XLSX) {
      ...
      reader.onload = (event) => {
        ...
        const datasets = workbook.SheetNames.map((name) => ({
          name,
          rows: window.XLSX.utils.sheet_to_json(workbook.Sheets[name], { defval: "" })
        })).filter((item) => item.rows.length);
        useUploadedDatasets(datasets, file.name);
      };
      reader.readAsArrayBuffer(file);
      return;
    }
    state.uploadNote = /\.xlsx?$/i.test(file.name) ? "当前环境没有成功加载 Excel 解析器，请先使用 CSV，或在线上环境直接上传 Excel。" : "暂不支持该文件类型，请上传 Excel 或 CSV。";
    render();
  }
  ...
  function downloadReport() {
    ...
    link.href = url;
    link.download = "澄曜云服科技-V8组织诊断汇报摘要.txt";
    link.click();
  ...
  function bindEvents() {
    ...
    document.querySelectorAll('[data-action="open-analysis"]').forEach((node) => { node.onclick = () => { if (!currentAnalysisDepth().canAnalyze) return; state.route = "overview"; window.location.hash = "overview"; render(); }; });
  ...
  function syncRoute() {
    ...
    const valid = visibleRoutes().some((route) => route.key === hash) || hash === 'profile';
    state.route = valid ? (hash || 'home') : (hasEmployeeData() ? 'overview' : 'home');
    render();
  }
