(function () {
  ...
  function buildDiagnostics(input) {
    const roleRows = (input.roles && input.roles.length ? input.roles : input.criticalRoles || []);
    const signals = buildSignals(input.rows || [], input.departmentSummary || {}, roleRows, input.workforceMonthly || [], input.recruitingReqs || []);
  ...
  window.TalentPulseAnalysisV8 = { buildDiagnostics };
})();
